...
window.saveNewProduct=async()=>{
  ...
  if(sku&&state.products.some(p=>p.sku===sku)) return toast('SKU ซ้ำ');
  const productDoc=doc(userPath('products')),logDoc=doc(logRef()),auditDoc=doc(auditRef()),eventId=makeEventId('PRODUCT');
  const product={name,sku,category:$('pc').value,unit:$('pu').value||'ชิ้น',stock:Number($('pq').value)||0,min:Number($('pm').value)||0,archived:false,photo:'',createdAt:serverTimestamp(),updatedAt:serverTimestamp()};
  const batch=writeBatch(fs);
  batch.set(productDoc,product);
  batch.set(logDoc,logPayload('เพิ่มสินค้า',name,{productId:productDoc.id,eventId}));
  batch.set(auditDoc,auditPayload('เพิ่มสินค้า',name,{productId:productDoc.id,eventId,logId:logDoc.id}));
  try{ await batch.commit(); hideModal(); toast('เพิ่มสินค้าแล้ว'); }catch(e){ console.error(e); toast('เพิ่มสินค้าไม่สำเร็จ'); }
};
...
window.saveEditProduct=async(id)=>{
  ...
  if(!changes.length) return toast('ไม่มีการเปลี่ยนแปลงข้อมูล');
  const eventId=makeEventId('PRODUCT'),logDoc=doc(logRef()),auditDoc=doc(auditRef()),batch=writeBatch(fs);
  batch.update(productRef(id),{name,sku,category,unit,min,updatedAt:serverTimestamp()});
  batch.set(logDoc,logPayload('แก้ไขสินค้า',name,{productId:id,changes,eventId}));
  batch.set(auditDoc,auditPayload('แก้ไขสินค้า',name,{productId:id,changes,eventId,logId:logDoc.id}));
  try{ await batch.commit(); hideModal(); toast('แก้ไขสินค้าแล้ว'); }catch(e){ console.error(e); toast('แก้ไขสินค้าไม่สำเร็จ'); }
};
...
window.approve=async(id)=>{
  ...
  try{
    await runTransaction(fs,async tx=>{
      ...
      let logId=a.logId||'';
      if(logId){
        const lRef=logDocRef(logId),lSnap=await tx.get(lRef);
        if(lSnap.exists()) tx.update(lRef,finalFields);
        else { const fallback=doc(logRef()); logId=fallback.id; tx.set(fallback,logPayload('อนุมัติ',detail,finalFields)); }
      }else{
        const fallback=doc(logRef()); logId=fallback.id; tx.set(fallback,logPayload('อนุมัติ',detail,finalFields));
      }
  ...
  try{
    await runTransaction(fs,async tx=>{
      ...
      if(logId){
        ...
        const fallback=doc(logRef()); logId=fallback.id; tx.set(fallback,logPayload('ปฏิเสธ',detail,finalFields));
      }
      const auditDoc=doc(auditRef());
  ...
  try{
    await runTransaction(fs,async tx=>{
      ...
      if(logId){
        ...
        const fallback=doc(logRef()); logId=fallback.id; tx.set(fallback,logPayload('ยกเลิก',detail,finalFields));
      }
      const auditDoc=doc(auditRef());
...
window.saveApproval=async(id)=>{
  ...
  try{
    await runTransaction(fs,async tx=>{
      ...
      if(logId){
        const lRef=logDocRef(logId),lSnap=await tx.get(lRef);
        if(lSnap.exists()) tx.update(lRef,logFields);
        ...
        const fallback=doc(logRef()); logId=fallback.id; tx.set(fallback,logPayload('แก้ไขรายการรออนุมัติ',detail,{...logFields,productId:a.productId})); tx.update(aRef,{logId});
      }
      const auditDoc=doc(auditRef());
