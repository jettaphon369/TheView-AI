...
function refreshHeaderMenu(){
  ...
  $('headerExcelBtn').onclick=()=>{ panel.classList.add('hidden'); window.exportReportCSV(); };
  $('headerPdfBtn').onclick=()=>{ panel.classList.add('hidden'); window.printReportPDF(); };
...
window.exportBackup=()=>{
  ...
  const data={version:'30.6-normalize-date-fix',workspace:'main',products:clean(state.products),approvals:clean(state.approvals),logs:clean(state.logs),auditLogs:clean(state.auditLogs),exportedAt:new Date().toISOString(),exportedBy:state.profile?.displayName||state.profile?.username||''};
