...
const BUILD_VERSION='34.16.0-EXPIRY-PHASE1';
...
onAuthStateChanged(auth, async user=>{
  ...
  try{
    ...
    // v34.15.6: กู้สิทธิ์บัญชีที่มี passwordChangePending ค้าง
    // pending ต้องมีผลเฉพาะระหว่างที่ mustChangePassword=true เท่านั้น
    if(!mustChangePassword && state.profile.passwordChangePending === true){
      try{
        await updateDoc(memberRef(user.uid),{
          passwordChangePending:false,
          passwordGateRecoveredAt:serverTimestamp()
        });
        state.profile.passwordChangePending=false;
      }catch(recoverError){
        // Rules รุ่นใหม่ไม่ใช้ pending เป็นตัวบล็อก จึงยังเข้าสู่ระบบได้แม้การล้าง field ไม่สำเร็จ
        console.warn('ล้าง passwordChangePending ที่ค้างไม่สำเร็จ',recoverError);
      }
    }
...
window.saveNewProduct=async()=>{
  ...
  const product={name,sku,skuKey:normalizeSkuKey(sku),category:$('pc').value,unit:$('pu').value||'ชิ้น',stock:Number($('pq').value)||0,min:Number($('pm').value)||0,...expiry,archived:false,photo:'',createdAt:serverTimestamp(),updatedAt:serverTimestamp()};
  try{
