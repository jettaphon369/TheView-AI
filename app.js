...
['cameraInput','photoInput'].forEach(id=>$(id).onchange=async e=>{ const f=e.target.files[0]; if(!f) return; state.selectedImage = await compressImage(f); e.target.value=''; renderScan(); toast('เลือกรูปแล้ว (บีบอัดอัตโนมัติ)'); });
...
window.exportBackup=()=>{
  ...
  const data={version:'32.4-remove-ai-assist',workspace:'main',products:clean(state.products),approvals:clean(state.approvals),logs:clean(state.logs),auditLogs:clean(state.auditLogs),exportedAt:new Date().toISOString(),exportedBy:state.profile?.displayName||state.profile?.username||''};
