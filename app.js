...
const BUILD_VERSION='34.15.2-RESET-COMPLETE-FIX';
...
async function deleteCollectionInChunks(name,onProgress){
  ...
  if(!verify.empty) throw new Error(`ลบ ${name} ไม่ครบ ยังเหลือ ${verify.size} รายการ`);
  return docs.length;
}
async function deleteProductStorageFiles(){
  const snap=await getDocsFromServer(userPath('products'));
  const paths=[...new Set(snap.docs.flatMap(d=>{
    const data=d.data()||{};
    return [data.photoPath,data.imagePath,data.storagePath].filter(Boolean);
  }))];
  let deleted=0;
...
window.executeReset=async(type)=>{
  ...
  try{
    ...
    if(type==='usage'||type==='history'||type==='factory'){
      state.approvals=[]; state.logs=[]; state.auditLogs=[];
      logsCursor=null; logsHasMore=true; logsLoadingMore=false;
    }
    localStorage.removeItem(PRODUCT_DETAIL_KEY);
