...
// ---------- ปุ่มรูปตา แสดง/ซ่อนรหัสผ่าน ใช้ได้ทั้งมือถือและคอม ----------
function ensurePasswordToggleStyles(){
  if(document.getElementById('theviewPasswordToggleStyles')) return;
  const style=document.createElement('style');
  style.id='theviewPasswordToggleStyles';
  style.textContent=`
    .password-field-wrap{position:relative;width:100%}
    .password-field-wrap>input{width:100%;padding-right:52px!important;box-sizing:border-box}
    .password-toggle-btn{position:absolute;right:8px;top:50%;transform:translateY(-50%);width:40px;height:40px;border:0;border-radius:12px;background:transparent;display:flex;align-items:center;justify-content:center;font-size:20px;cursor:pointer;z-index:2;color:#334155}
    .password-toggle-btn:hover{background:#eef2f7}
    .password-toggle-btn:focus-visible{outline:2px solid #60a5fa;outline-offset:2px}
  `;
  document.head.appendChild(style);
}
function attachPasswordToggles(root=document){
  ensurePasswordToggleStyles();
  const ids=['password','firstNewPass','firstConfirmPass','currentPass','newPass','confirmPass'];
  ids.forEach(id=>{
    const input=root.querySelector?.(`#${id}`) || document.getElementById(id);
    ...
    btn.addEventListener('click',()=>{
      ...
      btn.setAttribute('title',showing?'แสดงรหัสผ่าน':'ซ่อนรหัสผ่าน');
      input.focus();
...
window.saveNewPassword=async(first)=>{
  ...
  try{
    // ตั้งรหัสผ่านครั้งแรก: ผู้ใช้เพิ่งล็อกอินสำเร็จแล้ว จึงไม่ต้องยืนยันด้วยรหัสเริ่มต้นซ้ำ
    // การยืนยันรหัสปัจจุบันใช้เฉพาะการเปลี่ยนรหัสจากหน้าโปรไฟล์เท่านั้น
    if(!first){
...
// เพิ่มปุ่มรูปตาในหน้าเข้าสู่ระบบทันทีหลัง DOM พร้อมใช้งาน
