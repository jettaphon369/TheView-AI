...
window.addProduct=()=>{ if(!requireManager()) return; openModal('เพิ่มสินค้า',`
  ...
  <button id="saveNewProductBtn" class="btn primary full" onclick="window.saveNewProduct()">บันทึก</button>
`); };
window.saveNewProduct=async()=>{
  ...
  if(sku&&hasDuplicateSkuLocal(sku)){ endActionLock(lockKey,'saveNewProductBtn'); return toast('SKU ซ้ำ — กรุณาใช้รหัสอื่น'); }
  ...
  try{
    ...
    hideModal(); toast('เพิ่มสินค้าแล้ว');
...
window.saveEditProduct=async(id)=>{
  ...
  if(sku && hasDuplicateSkuLocal(sku,id)){ endActionLock(lockKey,'saveEditProductBtn'); return toast('SKU ซ้ำ — กรุณาใช้รหัสอื่น'); }
