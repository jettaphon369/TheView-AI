...
function saveCurrentPageScroll(){
  ...
  map[state.page]=window.scrollY||document.documentElement.scrollTop||0;
  localStorage.setItem(LAST_SCROLL_MAP_KEY,JSON.stringify(map));
  localStorage.setItem(LAST_SCROLL_KEY,String(map[state.page]||0));
}
...
function goToPage(page, opts={}){
  ...
  if(state.page==='stock') state.stockFilter = opts.filter || state.stockFilter || 'all';
...
window.exportBackup=()=>{
  ...
  const data={version:'32.5-state-persistence-draft',workspace:'main',products:clean(state.products),approvals:clean(state.approvals),logs:clean(state.logs),auditLogs:clean(state.auditLogs),exportedAt:new Date().toISOString(),exportedBy:state.profile?.displayName||state.profile?.username||''};
