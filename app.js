...
function getCurrentReportRows(){
  ...
  return filtered.map(l=>{
    ...
    return {
      ...
      status:l.status==='approved'?'อนุมัติ':l.status==='rejected'?'ปฏิเสธ':l.status==='pending'?'รออนุมัติ':l.status==='cancelled'?'ยกเลิก':(l.status||l.action||'')
    };
...
window.exportReportCSV=()=>{
  ...
  const createdAt=new Date().toLocaleString('th-TH');
  const headers=['วันที่','เวลา','ประเภท','สินค้า','จำนวน','หน่วย','สถานที่','ผู้ทำรายการ','ผู้อนุมัติ/ผู้ปฏิเสธ','สถานะ'];
  const lines=[
