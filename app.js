    ...
    ${state.approvals.map(a=>{
      ...
      return `<div class="card approval-card">
        ...
        ${canReview?`<div class="approval-sender-card">
          <div class="approval-sender-avatar">👤</div>
...
window.exportBackup=()=>{
  ...
  const data={version:'28.4',workspace:'main',products:clean(state.products),approvals:clean(state.approvals),logs:clean(state.logs),exportedAt:new Date().toISOString(),exportedBy:state.profile?.displayName||state.profile?.username||''};
