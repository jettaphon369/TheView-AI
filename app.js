...
function renderHome(){
  ...
  const todayLogs=state.logs
    ...
    .filter(l=>l._d && toDateStr(l._d)===todayKey && isStockMovementLog(l));
  const receiveToday=todayLogs.filter(isReceiveLog);
  const withdrawToday=todayLogs.filter(isWithdrawLog);
  const sumQty=(logs)=>logs.reduce((sum,l)=>sum+(Number(l.qty)||0),0);
  const greetingHour=new Date().getHours();
  const greeting=greetingHour<12?'สวัสดีตอนเช้า':greetingHour<18?'สวัสดีตอนบ่าย':'สวัสดีตอนเย็น';
  const displayName=state.profile?.firstName||state.profile?.displayName||state.profile?.username||'สมาชิก';

  const last7=[];
  for(let i=6;i>=0;i--){
    const d=new Date(); d.setDate(d.getDate()-i);
    const key=toDateStr(d);
    const logs=state.logs.map(l=>({...l,_d:getLogDate(l)})).filter(l=>l._d&&toDateStr(l._d)===key&&isStockMovementLog(l));
    last7.push({
      label:d.toLocaleDateString('th-TH',{weekday:'short'}),
      receive:logs.filter(isReceiveLog).length,
      withdraw:logs.filter(isWithdrawLog).length
    });
  }
  const maxBar=Math.max(1,...last7.flatMap(x=>[x.receive,x.withdraw]));
  const bars=last7.map(x=>`<div style="flex:1;min-width:34px;text-align:center"><div style="height:76px;display:flex;align-items:flex-end;justify-content:center;gap:4px"><span title="รับเข้า ${x.receive}" style="display:block;width:9px;height:${Math.max(4,Math.round(x.receive/maxBar*70))}px;border-radius:8px 8px 2px 2px;background:#16a34a"></span><span title="เบิกออก ${x.withdraw}" style="display:block;width:9px;height:${Math.max(4,Math.round(x.withdraw/maxBar*70))}px;border-radius:8px 8px 2px 2px;background:#f59e0b"></span></div><small class="muted">${x.label}</small></div>`).join('');

  const lowRows=lowItems.slice(0,5).map(p=>`<div class="between" style="padding:10px 0;border-bottom:1px solid var(--line)"><div><b>${escapeHtml(p.name)}</b><div class="muted">คงเหลือ ${p.stock} ${escapeHtml(p.unit||'')}</div></div><span class="pill warn">เตือน ${p.min}</span></div>`).join('');

  view.innerHTML=`
    <section style="margin-bottom:18px">
      <p class="muted" style="margin:0 0 4px">${greeting}</p>
      <h1 style="margin:0">${escapeHtml(displayName)} 👋</h1>
      <p class="muted" style="margin:6px 0 0">ภาพรวมระบบสต๊อกวันนี้</p>
    </section>

    <div class="grid">
      <div class="stat clickable" onclick="window.goToPage('stock')"><span>สินค้าทั้งหมด</span><b>${active.length}</b><small>รายการ</small></div>
      <div class="stat clickable" onclick="window.goToPage('approval')"><span>รอตรวจ</span><b>${state.approvals.length}</b><small>รายการ</small></div>
      <div class="stat clickable" onclick="window.goToPage('stock',{filter:'low'})"><span>ใกล้หมด</span><b style="color:${lowItems.length?'#dc2626':'#16a34a'}">${lowItems.length}</b><small>รายการ</small></div>
      <div class="stat clickable" onclick="window.goToPage('report')"><span>รายการวันนี้</span><b>${todayLogs.length}</b><small>ครั้ง</small></div>
    </div>

    <div class="card">
      <div class="between"><h2 style="margin:0">วันนี้</h2><button class="btn small light" onclick="window.goToPage('report')">ดูรายงาน</button></div>
      <div class="grid" style="margin-top:12px">
        <div class="stat"><span>📥 รับเข้า</span><b style="color:#16a34a">${receiveToday.length}</b><small>${sumQty(receiveToday)} หน่วยรวม</small></div>
        <div class="stat"><span>📤 เบิกออก</span><b style="color:#b45309">${withdrawToday.length}</b><small>${sumQty(withdrawToday)} หน่วยรวม</small></div>
      </div>
    </div>

    <div class="card">
      <div class="between"><h2 style="margin:0">ปุ่มลัด</h2></div>
      <div class="grid" style="margin-top:12px">
        ${isManager()?`<button class="btn primary" onclick="window.goToPage('stock');setTimeout(()=>window.addProduct(),0)">➕ เพิ่มสินค้า</button>`:''}
        <button class="btn light" onclick="window.goToPage('scan')">📷 AI Assist</button>
        <button class="btn light" onclick="window.goToPage('approval')">✅ ตรวจสอบ</button>
        <button class="btn light" onclick="window.goToPage('report')">📊 รายงาน</button>
      </div>
    </div>

    <div class="card">
      <div class="between"><h2 style="margin:0">กิจกรรม 7 วันล่าสุด</h2><div class="muted" style="font-size:12px"><span style="color:#16a34a">●</span> รับเข้า &nbsp; <span style="color:#f59e0b">●</span> เบิกออก</div></div>
      <div style="display:flex;gap:8px;align-items:flex-end;margin-top:14px;overflow-x:auto">${bars}</div>
    </div>

    <div class="card">
      <div class="between"><h2 style="margin:0">สินค้าใกล้หมด</h2><button class="btn small light" onclick="window.goToPage('stock',{filter:'low'})">ดูทั้งหมด</button></div>
      <div style="margin-top:8px">${lowRows||'<p class="muted" style="margin-bottom:0">✅ ยังไม่มีสินค้าใกล้หมด</p>'}</div>
    </div>`;
}
...
function roleLabel(value='staff'){
  return ({admin:'Administrator',manager:'Manager',staff:'Staff'}[value]||value||'Staff');
}
  ...
  view.innerHTML = `<div class="profile-page">
    ...
    ${isAdmin()?`<section class="profile-section">
      ...
      <div class="profile-action-grid">
        <button class="profile-action primary" onclick="window.manageMembers()">👥 จัดการสมาชิก</button>
    ...
    </section>`:`<section class="profile-section"><button class="profile-action full" onclick="window.viewTrash()">🗑️ ถังขยะ${trashCount?` (${trashCount})`:''}</button></section>`}

    <details class="profile-section profile-audit">
      <summary>📋 ประวัติการใช้งานล่าสุด</summary>
      <div class="profile-log-list">${auditRows}</div>
    </details>
  </div>`;
}
window.saveProfileDetails=async()=>{
  ...
  try{
    ...
    console.error(e);
    toast(`บันทึกโปรไฟล์ไม่สำเร็จ (${e?.code||'unknown'})`);
  }finally{
...
window.exportBackup=()=>{
  ...
  const data={version:'25.9',workspace:'main',products:clean(state.products),approvals:clean(state.approvals),logs:clean(state.logs),exportedAt:new Date().toISOString(),exportedBy:state.profile?.displayName||state.profile?.username||''};
