...
// ---------- v28.7 PC mouse-wheel scrolling fix ----------
function ensureDesktopWheelScrollV287(){
  if(window.__theviewDesktopWheelV287) return;
  window.__theviewDesktopWheelV287=true;
  ...
  document.addEventListener('wheel',(event)=>{
    ...
    // Modal and dropdowns keep their own scrolling.
    if(target.closest('.sheet,.scan-product-results,input,select,textarea')) return;

    const fixedArea=target.closest('.hero,.bottom-nav');
    if(!fixedArea) return;

    // Let the sidebar itself scroll when it still has room.
    if(fixedArea.classList.contains('bottom-nav')){
      const down=event.deltaY>0;
      const canDown=fixedArea.scrollTop+fixedArea.clientHeight<fixedArea.scrollHeight-1;
      const canUp=fixedArea.scrollTop>1;
...
function renderHome(){
  ...
  view.innerHTML=`<h1>หน้าแรก</h1>
    ...
    <div class="grid home-dashboard-grid">
      ...
      <div class="stat clickable" onclick="window.goToPage('history')"><span>ประวัติ</span><b>${state.logs.length}</b></div>
