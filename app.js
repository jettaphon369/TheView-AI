...
const BUILD_VERSION='34.16.3-DESKTOP';
...
// ---------- v34.16.3 Dedicated Desktop Workspace ----------
function ensureDesktopWorkspaceV34163(){
  if(!document.getElementById('theviewDesktopWorkspaceV34163')){
    ...
    style.textContent=`
      ...
      @media (min-width:1024px){
        :root{--desktop-sidebar:292px;--desktop-brand-height:214px;--desktop-topbar:76px}
        ...
        .hero{
          ...
          margin:0!important;padding:22px 24px 18px!important;border-radius:0!important;overflow:hidden!important;
          ...
          box-shadow:12px 0 34px rgba(16,43,35,.12)!important;z-index:120!important
        }
        .hero:before{opacity:.58!important;border-radius:0!important}
        .hero:after{display:none!important}
        .hero-shell{display:block!important;width:100%!important;min-height:0!important;padding:0!important}
        .hero-topline{display:block!important;min-height:0!important;padding:0!important}
        .hero-resort-brand{display:block!important;width:100%!important;max-width:none!important;height:70px!important;margin:0!important;overflow:visible!important}
        .brand-logo-hero{display:block!important;width:178px!important;height:70px!important;margin:0!important;object-fit:contain!important;object-position:left center!important;filter:brightness(0) invert(1) sepia(.28) saturate(.78)!important}
        .hero-main-copy{margin:17px 0 0!important;padding:0!important;text-align:left!important}
        .hero h1{display:block!important;margin:0!important;font-family:Georgia,"Times New Roman",serif!important;font-size:21px!important;line-height:1.12!important;font-weight:600!important;letter-spacing:.055em!important;text-align:left!important;white-space:normal!important;color:#fff!important}
        .hero-divider{justify-content:flex-start!important;gap:7px!important;margin:12px 0 10px!important}
        .hero-divider:before{width:62px!important}
        .hero-divider:after{width:112px!important}
        .hero-divider span{width:15px!important;height:15px!important;font-size:10px!important}
        .hero p{margin:0!important;padding:0!important;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif!important;font-size:11px!important;line-height:1.35!important;font-weight:700!important;letter-spacing:.015em!important;text-align:left!important;white-space:normal!important;color:#e4c77e!important}
        ...
        #headerMenuPanel.header-menu-panel{
          width:340px!important;min-width:340px!important;max-height:calc(100vh - 98px)!important;padding:10px!important;
          border-radius:18px!important;border:1px solid #dce5e0!important;background:#fff!important;
          box-shadow:0 24px 58px rgba(21,46,38,.2)!important;overflow-y:auto!important
        }
        #headerMenuPanel.header-menu-panel::before{display:none!important}
...
function refreshStockSearchResults(){
  ...
  const rows=list.map(p=>{
    ...
    return `<article class="stock-card-modern" style="--stock-accent:${accent}" role="button" tabindex="0" aria-label="ดูรายละเอียด ${escapeHtml(p.name)}" onclick="window.viewProduct('${p.id}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();window.viewProduct('${p.id}')}">
      <div class="stock-card-photo">${p.photo?productImageMarkup(p.photo,p.name):`<div class="stock-card-photo-placeholder">📦</div>`}</div>
      <div class="stock-card-main"><h3 class="stock-card-name">${escapeHtml(p.name)}</h3><div class="stock-card-sku">รหัสสินค้า: ${escapeHtml(p.sku||'-')}</div>${expiryBadge?`<div class="stock-card-expiry-row">${expiryBadge}<span>หมดอายุ ${escapeHtml(expiryDateLabel(p))}</span></div>`:''}<div class="stock-card-label">คงเหลือ</div><div class="stock-card-qty"><span class="stock-card-number">${stock}</span><span class="stock-card-unit">${escapeHtml(p.unit||'หน่วย')}</span></div></div>
      <div class="stock-card-side"><span class="stock-status-modern ${statusClass}">${statusText}</span><span class="stock-card-arrow">›</span></div>
    </article>`;
  }).join('');
...
window.saveNewProduct=async()=>{
  ...
  const product={name,sku,skuKey:normalizeSkuKey(sku),category:$('pc').value,unit:$('pu').value||'ชิ้น',stock:initialStock,min:minimumStock,...expiry,archived:false,photo:'',photoPath:'',createdAt:serverTimestamp(),updatedAt:serverTimestamp()};
  try{
...
window.sendApproval=async()=>{
  ...
  if(type==='out'&&!confirmExpiredIssue(p)){ endActionLock(lockKey,'sendApprovalBtn'); return; }
...
const RESET_PLANS={
  usage:{
    ...
    collections:['products','approvals','logs','auditLogs','skuRegistry','notifications'],
  ...
  factory:{
    ...
    collections:['products','approvals','logs','auditLogs','skuRegistry','notifications','settings'],
...
async function deleteOtherMemberProfiles(){
  ...
  if(state.user?.uid){
    await setDoc(memberRef(state.user.uid),{
      role:'admin',status:'active',permissions:{canApprove:true,canAdjustStock:true,canManageProducts:true,canViewReports:true},
