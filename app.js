...
window.openReportDetails=(type,location)=>{
  ...
  openModal(title,`<p class="muted">ช่วงเวลา: ${escapeHtml(period)} • ${logs.length} รายการ</p>${groups||'<p class="muted">ไม่พบรายละเอียด</p>'}<button class="btn full" onclick="closeModal()">ปิด</button>`);
};
