...
function appNameWithEnvHtml(){ return `${escapeHtml(appName())}${APP_ENV==='TEST'?' <span class="test-badge">TEST</span>':''}`; }
...
function applySystemBranding(input,opts={cache:true}){
  ...
  const pageTitle=`${b.systemName}${APP_ENV==='TEST'?' TEST':''}`;
...
const BUILD_VERSION='34.29.48-MANUAL-PC-USABILITY-FIX';
...
const PRODUCT_CACHE_DB='theview-stock-cache-v3-wjb-story';
const PRODUCT_CACHE_STORE='kv';
const PRODUCT_CACHE_KEY='products-v3-wjb-story';
...
// TEST environment: keep browser cache isolated under the wjb-story namespace.
...
@media (min-width:1024px){
  ...
  .hero h1{font-size:22px!important;line-height:1.16!important}
  .hero p{font-size:12px!important;line-height:1.45!important}
  .brand-logo-hero{width:150px!important;height:60px!important}
...
function renderProductDetail(id){
  ...
  const history=state.logs.filter(l=>l.productId===id).filter(l=>canAccessLogEntry(l)).sort((a,b)=>(getLogDate(b)?.getTime()||0)-(getLogDate(a)?.getTime()||0));
  const stock=Number(p.stock)||0;
  const min=Number(p.min)||0;
  const isOut=stock<=0;
  const isLow=!isOut&&stock<=min;
...
function stockStructureManagerMarkup(){
  ...
  const cards=groups.map(g=>{
    ...
    return `<article class="stock-structure-card ${g.status==='inactive'?'inactive':''}"><div class="stock-structure-head"><div><b>🏠 ${escapeHtml(g.name)}</b><small>ID: ${escapeHtml(g.id)}</small></div><span>${g.status==='inactive'?'ปิดใช้งาน':'ใช้งาน'}</span></div><div class="stock-area-list">${displayAreas.length?displayAreas.map(a=>`<div class="stock-area-row ${a.status==='inactive'?'inactive':''}"><span>↳ ${escapeHtml(a.name)}</span><small>${a.status==='inactive'?'ปิดใช้งาน':'ใช้งาน'}</small><button class="btn tiny" onclick="window.toggleStockAreaStatus('${escapeHtml(g.id)}','${escapeHtml(a.id)}')">${a.status==='inactive'?'เปิด':'ปิด'}</button></div>`).join(''):'<p class="muted">ยังไม่มีพื้นที่ในกลุ่มนี้</p>'}</div><div class="row"><button class="btn light small" onclick="window.prepareAddStockArea('${escapeHtml(g.id)}')">+ เพิ่มพื้นที่ในกลุ่มนี้</button>${g.id!==DEFAULT_STOCK_GROUP_ID?`<button class="btn small" onclick="window.toggleStockGroupStatus('${escapeHtml(g.id)}')">${g.status==='inactive'?'เปิดกลุ่ม':'ปิดกลุ่ม'}</button>`:''}</div></article>`;
  }).join('');
