...
function ensureAppShellStyles(){
  ...
  style.textContent=`
    ...
    .header-menu-panel{
      position:absolute;right:18px;top:78px;z-index:120;min-width:220px;
...
function refreshHeaderMenu(){
  ...
  panel.innerHTML=`
    <button type="button" id="headerProfileBtn">👤 โปรไฟล์ของฉัน</button>
    ${canManageProducts()?'<button type="button" id="headerAdjustBtn">⚖️ ปรับยอดสต๊อก</button><button type="button" id="headerAuditBtn">🛡️ Audit Log</button>':''}
    <button type="button" id="headerReportBtn">📊 รายงานสต๊อก</button>
    <button type="button" id="headerExcelBtn">📗 ดาวน์โหลด Excel/CSV</button>
    <button type="button" id="headerPdfBtn">📄 PDF / พิมพ์รายงาน</button>
    ${isAdmin()?'<button type="button" id="headerBackupBtn">⬇️ สำรองข้อมูล</button><button type="button" id="headerRestoreBtn">⬆️ กู้คืนข้อมูล</button>':''}
    <button type="button" id="headerLogoutBtn" class="logout-item">🚪 ออกจากระบบ</button>`;
  ...
  $('headerExcelBtn').onclick=()=>{ panel.classList.add('hidden'); goToPage('report'); setTimeout(()=>window.exportReportCSV(),100); };
  $('headerPdfBtn').onclick=()=>{ panel.classList.add('hidden'); goToPage('report'); setTimeout(()=>window.printReportPDF(),100); };
...
function renderStock(){
  ...
  list=[...list].sort((a,b)=>{
    ...
    if(sortMode==='low-first'){
      const aOut=Number(a.stock)<=0?0:(Number(a.stock)<=Number(a.min)?1:2);
      const bOut=Number(b.stock)<=0?0:(Number(b.stock)<=Number(b.min)?1:2);
      return aOut-bOut || String(a.name||'').localeCompare(String(b.name||''),'th');
    }
  ...
  view.innerHTML = `<div class="between stock-page-head">
    ...
    <div class="card stock-toolbar">
      ...
      <select id="stockSortSelect" class="stock-sort-field" onchange="window.setStockSort(this.value)">
        ...
        <option value="low-first" ${sortMode==='low-first'?'selected':''}>สินค้าหมด/ใกล้หมดก่อน</option>
      </select>
    ...
    ${filterLow?`<div class="card" style="display:flex;justify-content:space-between;align-items:center;gap:10px">
      ...
      <button class="btn small" onclick="window.goToPage('stock')">แสดงทั้งหมด</button>
    </div>`:''}
  ...
  view.innerHTML=`<div class="between">
    ...
    <section class="card new-item-form-card">
      ...
      <details class="new-item-attachment">
        <summary>📷 แนบรูปหรือใช้ AI Assist</summary>
        <div class="new-item-attachment-body">
          <p class="muted" style="margin-top:0">ใช้เมื่อต้องการแนบหลักฐานหรือให้ระบบช่วยจับคู่จากข้อความ</p>
          <div class="grid">
            <button type="button" class="btn primary" onclick="cameraInput.click()">📷 ถ่ายรูป</button>
            <button type="button" class="btn" onclick="photoInput.click()">🖼️ รูปภาพ</button>
            <button type="button" class="btn" onclick="fileInput.click()">📁 ไฟล์</button>
          </div>
          ${state.selectedImage?`<img class="preview" src="${state.selectedImage}">`:''}
          <input id="scanText" class="new-item-input" placeholder="เช่น เบิกแก้ว 22 oz 2 แพ็ค">
          <button type="button" class="btn light full" onclick="window.freeAssist()">ช่วยจับคู่จากข้อความ</button>
        </div>
      </details>
...
function renderReport(){
  ...
  <div class="card report-export-card"><div><b>ส่งออกรายงาน</b><div class="muted" style="font-size:13px">ใช้ช่วงเวลาและตัวกรองที่เลือกอยู่</div></div><div class="row report-export-actions"><button class="btn small green" onclick="window.exportReportCSV()">📗 Excel/CSV</button><button class="btn small primary" onclick="window.printReportPDF()">📄 PDF / พิมพ์</button></div></div>
...
function reportFileBase(){
  const period=state.reportMode==='month'?(state.reportMonth||toMonthStr(new Date())):(state.reportDate||toDateStr(new Date()));
...
window.printReportPDF=()=>{
  const rows=getCurrentReportRows();
  if(!rows.length){ alert('ไม่มีข้อมูลในช่วงเวลาที่เลือก'); return; }
  const period=state.reportMode==='month'?`เดือน ${state.reportMonth}`:`วันที่ ${state.reportDate}`;
...
function historyTypeKey(log){
  ...
  if(log.action==='รับเข้า') return 'in';
  if(log.action==='เบิกออก') return 'out';
  return 'other';
}
...
function renderHistory(){
  ...
  if(q){
    logs=logs.filter(l=>[
      l.action,l.detail,l.actorName,l.reviewerName,l.location,l.unit,l.qty
  ...
  const renderLog=(l)=>{
    ...
    const qty=l.qty ? `<strong class="history-qty">${Number(l.qty)||0} ${escapeHtml(l.unit||'')}</strong>` : '';
...
window.exportBackup=()=>{
  ...
  const data={version:'30.5-header-report-export',workspace:'main',products:clean(state.products),approvals:clean(state.approvals),logs:clean(state.logs),auditLogs:clean(state.auditLogs),exportedAt:new Date().toISOString(),exportedBy:state.profile?.displayName||state.profile?.username||''};
...
function openModal(t,b){ $('modalTitle').textContent=t; $('modalBody').innerHTML=b; $('modal').classList.remove('hidden'); document.body.classList.add('modal-open'); const sheet=$('modal').querySelector('.sheet'); if(sheet) sheet.scrollTop=0; refreshPasswordEyes($('modalBody')); } function hideModal(){ if(state.profile?.mustChangePassword) return; $('modal').classList.add('hidden'); document.body.classList.remove('modal-open'); }
