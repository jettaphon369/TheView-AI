...
const state = { user:null, profile:null, members:[], page:VALID_PAGES.has(savedPage)?savedPage:'home', products:[], approvals:[], logs:[], selectedImage:null, imageMode:null, viewProductId:null, tempMoveImage:null, tempProductImage:null, stockFilter:'all', stockSearch:'', stockSort:'name-asc', reportMode:'day', reportFilter:'all', reportDate:'', reportMonth:'' };
const view = $('view');

function toast(msg){ const t=$('toast'); t.textContent=msg; t.classList.add('show'); setTimeout(()=>t.classList.remove('show'),1800); }
const actionLocks=new Set();
function startAction(key){ if(actionLocks.has(key)){ toast('กำลังดำเนินการ กรุณารอสักครู่'); return false; } actionLocks.add(key); return true; }
function endAction(key){ actionLocks.delete(key); }
function setBusyButton(buttonId,busy,busyText='กำลังบันทึก...'){ const btn=$(buttonId); if(!btn) return; if(!btn.dataset.normalText) btn.dataset.normalText=btn.textContent; btn.disabled=busy; btn.textContent=busy?busyText:btn.dataset.normalText; }
...
`); };
window.saveNewProduct=async()=>{ const key='save-new-product'; if(!startAction(key)) return; try{ const name=$('pn').value.trim(), sku=$('ps').value.trim(); if(!name) return toast('กรอกชื่อสินค้า'); if(sku && state.products.some(p=>p.sku===sku)) return toast('SKU ซ้ำ'); const stock=Number($('pq').value)||0, min=Number($('pm').value)||0; if(stock<0||min<0) return toast('จำนวนต้องไม่ติดลบ'); await addDoc(userPath('products'),{name,sku,category:$('pc').value.trim(),unit:$('pu').value.trim()||'ชิ้น',stock,min,archived:false,trashed:false,photo:'',createdAt:serverTimestamp(),createdBy:state.user?.uid||''}); await addLog('เพิ่มสินค้า',name,{qty:stock,unit:$('pu').value.trim()||'ชิ้น'}); hideModal(); toast('เพิ่มสินค้าเรียบร้อย'); }catch(e){ console.error(e); toast(e?.message||'เพิ่มสินค้าไม่สำเร็จ'); }finally{ endAction(key); } };
...
window.applyStock=async(id,type)=>{
  const key=`stock-${type}-${id}`; if(!startAction(key)) return;
  const q=Number($('qty').value)||0;
  if(q<=0){ endAction(key); return toast('จำนวนไม่ถูกต้อง'); }
  const reason=($('reason').value||'').trim();
  const location=type==='in' ? STORE_LOCATION : getLocationValue('moveLoc','moveLocOther');
  if(type==='out' && !location){ endAction(key); return toast('กรุณาเลือกสถานที่เบิก'); }
  ...
  }catch(e){ console.error(e); toast(e?.message||'บันทึกสต๊อกไม่สำเร็จ'); } finally { endAction(key); }
};
...
window.sendApproval=async()=>{
  const key='send-approval'; if(!startAction(key)) return;
  try{
    ...
    if(type==='out' && !location) return toast('กรุณาเลือกสถานที่ก่อนส่งเข้าคิวตรวจ');
    const p=state.products.find(x=>x.id===productId);
    if(!p) return toast('ไม่พบสินค้า');
    if(type==='out' && qty>Number(p.stock)) return toast('เบิกเกินสต๊อก');
    const logDoc = await addLog('ส่งตรวจ',`${type==='out'?'เบิก':'รับ'} ${p.name} ${qty} ${p.unit}`,{productId,qty,unit:p.unit,photo:state.selectedImage||'',location,moveType:type});
    await addDoc(userPath('approvals'),{productId,name:p.name,qty,unit:p.unit,type,location,img:state.selectedImage||'',confidence:state.selectedImage?60:0,status:'pending',logId:logDoc.id,createdAt:serverTimestamp(),createdBy:state.user?.uid||''});
    state.selectedImage=null; renderScan(); toast('ส่งเข้าคิวตรวจแล้ว');
  }catch(e){ console.error(e); toast(e?.message||'ส่งเข้าคิวตรวจไม่สำเร็จ'); }
...
window.approve=async(id)=>{
  ...
  const key=`approve-${id}`; if(!startAction(key)) return;
  ...
  }catch(e){ console.error(e); toast(e?.message||'อนุมัติไม่สำเร็จ'); } finally { endAction(key); }
};
window.reject=async(id)=>{ if(!requireManager()) return; const key=`reject-${id}`; if(!startAction(key)) return; try{ const a=state.approvals.find(x=>x.id===id); if(!a) return toast('ไม่พบรายการ'); await deleteDoc(approvalRef(id)); if(a.logId){ await updateDoc(logDocRef(a.logId),{action:'ปฏิเสธ',time:new Date().toLocaleString('th-TH'),location:a.type==='in'?STORE_LOCATION:(a.location||''),reviewerUid:state.user?.uid||'',reviewerName:state.profile?.displayName||state.profile?.username||''}); } else { await addLog('ปฏิเสธ',a.name,{productId:a.productId,moveType:a.type,location:a.type==='in'?STORE_LOCATION:(a.location||'')}); } hideModal(); toast('ปฏิเสธรายการแล้ว'); }catch(e){ console.error(e); toast(e?.message||'ปฏิเสธรายการไม่สำเร็จ'); }finally{ endAction(key); } };
...
window.exportBackup=()=>{
  ...
  const data={version:'26.0-pilot',workspace:'main',products:clean(state.products),approvals:clean(state.approvals),logs:clean(state.logs),exportedAt:new Date().toISOString(),exportedBy:state.profile?.displayName||state.profile?.username||''};
...
// ---------- สถานะการเชื่อมต่อ ----------
window.addEventListener('offline',()=>toast('ออฟไลน์: กรุณารอการเชื่อมต่อก่อนบันทึกข้อมูล'));
