...
const BUILD_VERSION='34.22.1-MAIN-USER-MANUAL-CLEANUP';
...
function productQrPayload(product={}){
  ...
  url.searchParams.set('v','34.22.1');
  return url.toString();
}
...
function renderManual(){
  view.innerHTML = `<div class="manual-page visual-manual v34202">
    ...
    <section class="manual-role-grid visual-roles">
      <article class="role-admin"><div class="manual-role-icon">👑</div><h3>ผู้ดูแลระบบ</h3><p>ช่วยตรวจสิทธิ์ แก้ปัญหา Login และดูแลข้อมูลหลังบ้านเมื่อพบปัญหาที่ผู้ใช้งานแก้เองไม่ได้</p></article>
      <article class="role-lead"><div class="manual-role-icon">🧑‍💼</div><h3>Manager</h3><p>ดูภาพรวม อนุมัติ ตรวจรายงาน และตรวจรายการผิดปกติของแผนก</p></article>
    ...
    <section class="manual-section-list visual-section-list" id="manualSections">
      ...
      ${manualDetail('2. สิทธิ์ผู้ใช้งาน','role admin manager captain staff สิทธิ์','👥',`
        <div class="manual-role-task-grid">
          <div><b>ผู้ดูแลระบบ</b><ul><li>ช่วยตรวจสิทธิ์</li><li>แก้ปัญหา Login</li><li>ดูแลข้อมูลหลังบ้าน</li><li>ช่วยตรวจเมื่อพบข้อมูลผิด</li></ul></div>
          <div><b>Manager</b><ul><li>ดูภาพรวม</li><li>อนุมัติรายการ</li><li>ดูประวัติและรายงาน</li><li>ตรวจรายการผิดปกติ</li></ul></div>
          <div><b>Supervisor / Captain <span class="role-badge">สิทธิ์เท่ากัน</span></b><ul><li>รับเข้า / เบิกออก</li><li>อนุมัติรายการ</li><li>ตรวจล็อตและวันหมดอายุ</li><li>ดูประวัติและรายงาน</li></ul></div>
        ...
        <p class="manual-callout-warn">หากเข้าเมนูไม่ได้ หรือสิทธิ์ไม่ตรงกับหน้าที่ ให้แจ้งหัวหน้างานหรือผู้ดูแลระบบก่อนทำรายการจริง</p>` )}
      ...
      ${manualDetail('13. ข้อควรระวังก่อนบันทึกข้อมูลจริง','ข้อควรระวัง ข้อมูลจริง บันทึก จำนวน หน่วยนับ ล็อต สถานที่ รูปหลักฐาน','⚠️',`
        <ol class="manual-step-list"><li>ตรวจชื่อสินค้าให้ตรงกับสินค้าจริง</li><li>ตรวจจำนวนและหน่วยนับ เช่น จาน ขวด ชิ้น ลัง หรือแพ็ก</li><li>ตรวจล็อตและวันหมดอายุ หากสินค้านั้นมีล็อต</li><li>ตรวจสถานที่ใช้งานให้ถูกต้อง</li><li>แนบรูปหลักฐานให้ชัดเจนเมื่อมีการรับเข้า เบิกออก หรือข้อมูลผิดปกติ</li><li>อ่านข้อมูลทั้งหมดอีกครั้งก่อนกดบันทึก</li></ol>
    ...
    <section class="manual-summary">
      <h2>ข้อควรจำ</h2>
      <p>ก่อนบันทึกข้อมูลจริงทุกครั้ง ควรตรวจชื่อสินค้า จำนวน หน่วยนับ ล็อต สถานที่ และรูปหลักฐาน หากไม่แน่ใจให้แจ้งหัวหน้างานหรือผู้ดูแลระบบก่อนแก้ไขข้อมูลจริง</p>
    </section>
  </div>`;
}
...
window.manageMembers=async()=>{ if(!requireAdmin()) return;
  ...
  const rows=state.members.map(m=>`<div class="card member-row-card" style="box-shadow:none;border:1px solid var(--line);margin:8px 0">
    ...
    <div class="member-delete-note">ลบจาก CHEE CHAN STOCK เท่านั้น ไม่ลบ Email ใน Firebase Authentication</div>`}
  </div>`).join('');
...
ผลลัพธ์:
- สมาชิกจะหายจากรายชื่อใน CHEE CHAN STOCK
