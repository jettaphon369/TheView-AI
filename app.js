...
const BUILD_VERSION='34.15.1-RESET-CACHE-FIX';
...
const PRODUCT_CACHE_DB='theview-stock-cache-v2-theview-4d389';
const PRODUCT_CACHE_STORE='kv';
const PRODUCT_CACHE_KEY='products-theview-4d389';
...
async function clearProductImageRuntimeCache(){
  try{
    if(!('caches' in window)) return;
    const keys=await caches.keys();
    await Promise.all(keys.filter(k=>k.includes('theview-stock-runtime')).map(k=>caches.delete(k)));
  }catch(e){ console.warn('ล้าง Runtime cache ไม่สำเร็จ',e); }
}
...
async function deleteCollectionInChunks(name,onProgress){
  // ใช้ข้อมูลจาก Server โดยตรง เพื่อไม่ให้ IndexedDB cache ทำให้เห็นเอกสารที่ลบไปแล้ว
  let deleted=0;
  for(let pass=0;pass<3;pass++){
    ...
    for(let i=0;i<docs.length;i+=400){
      ...
      onProgress?.(deleted,deleted+Math.max(0,docs.length-i-400),name);
    }
    await waitForPendingWrites(fs);
  }
  const verify=await getDocsFromServer(userPath(name));
  if(!verify.empty) throw new Error(`RESET_VERIFY_FAILED:${name}:${verify.size}`);
  return deleted;
}
async function deleteProductStorageFiles(){
  const snap=await getDocsFromServer(userPath('products'));
  const paths=[...new Set(snap.docs.map(d=>d.data()?.imagePath).filter(Boolean))];
  let deleted=0;
...
window.executeReset=async(type)=>{
  if(!requireAdmin()||resetSystemRunning) return;
  if(!navigator.onLine){ toast('การรีเซ็ตต้องเชื่อมต่ออินเทอร์เน็ต'); return; }
  const plan=RESET_PLANS[type]; if(!plan) return;
  ...
  try{
    ...
    let step=0;
    const total=plan.collections.length+(type!=='history'?1:0)+(type==='factory'?2:0);
    if(type!=='history'){
      updateResetProgress('กำลังลบรูปสินค้า...',Math.round(step/total*100));
      await deleteProductStorageFiles(); step++;
    }
    for(const name of plan.collections){
      updateResetProgress(`กำลังลบ ${name}...`,Math.round(step/total*100));
      await deleteCollectionInChunks(name); step++;
    }
    if(type==='factory'){
      updateResetProgress('กำลังลบโปรไฟล์สมาชิกและสิทธิ์...',Math.round(step/total*100));
      await deleteOtherMemberProfiles(); step++;
      try{ await deleteDoc(doc(fs,'theviewWorkspaces','main')); }catch(_){ }
      step++;
    }
    // ล้างข้อมูลในหน่วยความจำและ IndexedDB หลังลบ Firestore สำเร็จ
    if(type!=='history'){
      state.products=[];
      productsSnapshotReady=true;
      await clearProductCache();
      await writeProductCache([],Date.now(),Date.now());
      await clearProductImageRuntimeCache();
    }
    state.approvals=[];
    state.logs=[];
    state.auditLogs=[];
    logsCursor=null; logsHasMore=false; logsLoadingMore=false;
    await waitForPendingWrites(fs);
    updateResetProgress('รีเซ็ตสำเร็จ',100);
    localStorage.removeItem(PRODUCT_DETAIL_KEY);
    state.viewProductId=null; state.page='home';
    render();
    setTimeout(()=>{ $('modal')?.classList.remove('reset-running'); hideModal(); window.goToPage('home',{resetScroll:true}); toast(`${plan.title}เรียบร้อยแล้ว`); },700);
  }catch(e){
    console.error('รีเซ็ตระบบไม่สำเร็จ',e);
    $('modal')?.classList.remove('reset-running');
    hideModal();
    toast(e?.code==='permission-denied'?'รีเซ็ตไม่ได้: สิทธิ์ Firebase ไม่อนุญาตบางรายการ':'รีเซ็ตไม่สำเร็จ กรุณาตรวจสอบอินเทอร์เน็ตแล้วลองใหม่');
