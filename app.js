...
const BUILD_VERSION='34.29.52-MULTI-PRODUCT-ADD';
...
function bulkProductRowMarkup(rowId){
  return `<section class="bulk-product-card" data-bulk-product-row="${rowId}">
    ...
    <div class="bulk-product-grid">
      <label>รหัสสินค้า (SKU)<input class="bulk-sku" placeholder="ไม่บังคับ" oninput="window.updateBulkProductCount()"></label>
...
function collectBulkProductRows(){
  return [...document.querySelectorAll('[data-bulk-product-row]')].map((card,index)=>{
    ...
    const hasAny=!!(name||sku||category||stockRaw||minRaw||hasExpiry||(unit&&unit!=='ชิ้น'));
    return {card,rowNumber:index+1,hasAny,name,sku,category,unit,initialStock:stockRaw===''?0:Number(stockRaw),minimumStock:minRaw===''?0:Number(minRaw),hasExpiry,expiryDate:hasExpiry?expiryDate:'',expiryReminderDays:hasExpiry?expiryReminderDays:0};
...
function validateBulkProductRows(rows,stockLoc){
  ...
  let valid=true;
  const names=new Map(),skus=new Map();
  const existingNames=new Set(state.products.filter(p=>!p.trashed&&!p.archived).map(p=>normalizeProductNameKey(p.name)));
  rows.forEach(row=>{
    let error='';
    const nameKey=normalizeProductNameKey(row.name),skuKey=normalizeSkuKey(row.sku);
    ...
    else if(row.sku && hasDuplicateSkuLocal(row.sku)) error='SKU นี้มีสินค้าอื่นใช้งานอยู่แล้ว';
    ...
    if(skuKey){
      if(skus.has(skuKey)){ setBulkProductRowError(row.card,'SKU ซ้ำกันในรายการนี้'); setBulkProductRowError(skus.get(skuKey).card,'SKU ซ้ำกันในรายการนี้'); valid=false; }
      else skus.set(skuKey,row);
    }
  });
...
window.addMultipleProducts=()=>{
  ...
  openModal('เพิ่มสินค้าหลายรายการ',`
    ...
    <div class="bulk-product-intro"><b>เพิ่มพร้อมกันได้สูงสุด 50 รายการ</b><span>สินค้าทั้งหมดด้านล่างจะใช้กลุ่ม/พื้นที่สต๊อกเดียวกัน และบันทึกพร้อมกันเป็นชุดเดียว</span></div>
    ...
    <p class="field-hint bulk-product-photo-note">📷 โหมดหลายสินค้ายังไม่บังคับรูป สามารถเข้าไปเพิ่มรูปของแต่ละสินค้าได้ภายหลัง</p>
...
window.saveMultipleProducts=async()=>{
  ...
  const items=rows.map(row=>{
    ...
    return {...row,productDoc,initialLot,regRef:row.sku?skuRegistryDocRef(row.sku):null};
  });
  try{
    await runTransaction(fs,async tx=>{
      const registryItems=items.filter(item=>item.regRef);
      const registrySnaps=new Map(await Promise.all(registryItems.map(async item=>[item.regRef.path,await tx.get(item.regRef)])));
      for(const item of items){
        if(item.regRef && registrySnaps.get(item.regRef.path)?.exists()) throw Object.assign(new Error(`SKU ${item.sku} ถูกใช้งานแล้ว (${item.name})`),{code:'bulk/sku-duplicate',rowNumber:item.rowNumber});
      }
      for(const item of items){
        ...
        if(item.regRef) tx.set(item.regRef,{sku:item.sku,skuKey:normalizeSkuKey(item.sku),productId:item.productDoc.id,createdAt:serverTimestamp(),updatedAt:serverTimestamp()});
      }
      ...
      const common={productId:'',productIds,productNames,bulkBatchId,bulkAdd:true,bulkCount:items.length,...stockLoc};
    ...
    hideModal();
    toast(`เพิ่มสินค้า ${items.length} รายการเรียบร้อย`);
  }catch(e){
    console.error('เพิ่มสินค้าหลายรายการไม่สำเร็จ',e);
    if(e?.code==='bulk/sku-duplicate') toast(e.message||'พบ SKU ซ้ำในฐานข้อมูล');
    else if(e?.code==='permission-denied') toast('ไม่มีสิทธิ์บันทึกสินค้า กรุณาตรวจ Firestore Rules');
