    ...
    ${state.approvals.map(a=>{
      ...
      return `<div class="card approval-card">
        ...
        ${canReview?`<div class="approval-actions">
          ...
          <button class="btn primary own-approval-button" onclick="window.editApproval('${a.id}')">
            <span class="own-approval-button-icon">✎</span>
            <span>แก้ไขรายการ</span>
          </button>
          <button class="btn red own-approval-button" onclick="window.confirmCancelApproval('${a.id}')">
            <span class="own-approval-button-icon">🗑</span>
...
window.exportBackup=()=>{
  ...
  const data={version:'28.1',workspace:'main',products:clean(state.products),approvals:clean(state.approvals),logs:clean(state.logs),exportedAt:new Date().toISOString(),exportedBy:state.profile?.displayName||state.profile?.username||''};
