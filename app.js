...
import { getFirestore, collection, doc, setDoc, updateDoc, deleteDoc, onSnapshot, addDoc, serverTimestamp, query, where, orderBy, getDocs, getDoc, writeBatch, runTransaction } from 'https://www.gstatic.com/firebasejs/10.12.5/firebase-firestore.js';
...
(function verifyBuildConsistency(){
  ...
  if(htmlBuild && htmlBuild!==BUILD_VERSION){
    ...
    try{
      const key='theview:build-reload';
...
const LAST_SCROLL_MAP_KEY='theview:scrollByPage';
const NEW_ITEM_DRAFT_KEY='theview:newItemDraft';
const UI_STATE_KEY='theview:uiState';
...
const state = { user:null, profile:null, members:[], page:VALID_PAGES.has(savedPage)?savedPage:'home', products:[], approvals:[], logs:[], auditLogs:[], selectedImage:null, imageMode:null, viewProductId:null, tempMoveImage:null, tempProductImage:null, stockFilter:'all', stockSearch:'', stockSort:'name-asc', stockCategory:'all', balanceCategory:'all', reportMode:'day', reportFilter:'all', reportDate:'', reportMonth:'', reportStart:'', reportEnd:'', historySearch:'', historyFilter:'all', historyStart:toDateStr(new Date()), historyEnd:toDateStr(new Date()) };
try{
  const savedUi=JSON.parse(localStorage.getItem(UI_STATE_KEY)||'{}');
  ['stockFilter','stockSort','stockCategory','balanceCategory','reportMode','reportFilter','reportDate','reportMonth','reportStart','reportEnd','historySearch','historyFilter','historyStart','historyEnd','newItemType'].forEach(k=>{ if(savedUi[k]!==undefined) state[k]=savedUi[k]; });
}catch(_){ }
function saveUiState(){
  const keys=['stockFilter','stockSort','stockCategory','balanceCategory','reportMode','reportFilter','reportDate','reportMonth','reportStart','reportEnd','historySearch','historyFilter','historyStart','historyEnd','newItemType'];
...
function canAdjustStock(){ return isAdmin() || isManagerRole() || isCaptain() || hasPermission('canAdjustStock') || hasPermission('canManageProducts'); }
...
async function addAudit(action,detail,extra={}){
  return addDoc(auditRef(),{
    ...
    actorName:state.profile?.displayName||state.profile?.username||'ไม่ทราบผู้ใช้',
    actorRole:role(), immutable:true, ...extra
  });
}
async function addLog(action,detail,extra={}){
  const payload={action,detail,time:new Date().toLocaleString('th-TH'),createdAt:serverTimestamp(),actorUid:state.user?.uid||'',actorName:state.profile?.displayName||state.profile?.username||'ไม่ทราบผู้ใช้',...extra};
  const result=await addDoc(logRef(),payload);
  try{ await addAudit(action,detail,extra); }catch(e){ console.warn('audit log failed',e); }
  return result;
}
...
function bindRealtime(){
  ...
  if(canManageProducts()){
    unsubAudit=onSnapshot(query(userPath('auditLogs'),orderBy('createdAt','desc')),snap=>{state.auditLogs=snap.docs.map(d=>({id:d.id,...d.data()}));},err=>console.warn('โหลด Audit Log ไม่สำเร็จ',err));
  }
}
...
function refreshHeaderMenu(){
  ...
  panel.innerHTML=`
    ...
    ${canManageProducts()?`<div class="header-menu-group">
      <div class="header-menu-group-title">จัดการสต๊อก</div>
      <button type="button" id="headerAdjustBtn">⚖️ ปรับยอดสต๊อก</button>
      <button type="button" id="headerAuditBtn">🛡️ Audit Log</button>
    </div>`:''}
...
function renderHome(){
  ...
  view.innerHTML=`<div class="dashboard-heading"><div><div class="dashboard-kicker">ภาพรวมคลังสินค้า</div><h1>หน้าแรก</h1></div><div class="dashboard-date">${new Date().toLocaleDateString('th-TH',{day:'numeric',month:'long',year:'numeric'})}</div></div>
    ...
    <section class="card dashboard-alert-card">
      ...
      <div class="dashboard-low-list">${lowItems.slice(0,5).map(p=>`<button onclick="window.viewProduct('${p.id}')"><span>${p.photo?`<img src="${p.photo}" alt="">`:'📦'}</span><div><b>${escapeHtml(p.name)}</b><small>จุดเตือน ${Number(p.min)||0} ${escapeHtml(p.unit||'')}</small></div><strong>${Number(p.stock)||0}</strong></button>`).join('')||'<div class="dashboard-empty ok">✅ ไม่มีสินค้าใกล้หมด</div>'}</div>
    </section>`;
}
...
function renderStock(){
    ...
    return `<article class="stock-card-modern"
      ...
      <div class="stock-card-photo">
        ${p.photo
          ...
          : `<div class="stock-card-photo-placeholder">📦</div>`}
      </div>
  ...
  view.innerHTML = `<div class="between stock-page-head">
      ...
      ${isManager()?`<div class="stock-head-actions"><button class="btn light small" onclick="window.openStockAdjustmentPicker()">⚖️ ปรับยอด</button><button class="btn primary small" onclick="window.addProduct()">+ เพิ่ม</button></div>`:''}
    </div>
...
function refreshStockSearchResults(){
  ...
  const rows=list.map(p=>{
    ...
    return `<article class="stock-card-modern" style="--stock-accent:${accent}" role="button" tabindex="0" aria-label="ดูรายละเอียด ${escapeHtml(p.name)}" onclick="window.viewProduct('${p.id}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();window.viewProduct('${p.id}')}">
      <div class="stock-card-photo">${p.photo?`<img src="${p.photo}" alt="${escapeHtml(p.name)}">`:`<div class="stock-card-photo-placeholder">📦</div>`}</div>
...
function renderProductDetail(id){
  ...
  if(!p){state.page='stock';renderStock();return;}
    ...
    <section class="product-detail-card">
      <div class="product-detail-top">
        <div class="product-detail-photo-wrap">
          ${p.photo?`<img src="${p.photo}" class="product-detail-photo" alt="${escapeHtml(p.name)}">`:`<div class="product-detail-photo-placeholder">📦</div>`}
      ...
      <div class="product-detail-tabs" role="tablist">
        <button type="button" class="product-detail-tab active" data-product-tab="general" onclick="window.switchProductDetailTab('general')">ข้อมูลทั่วไป</button>
        <button type="button" class="product-detail-tab" data-product-tab="movement" onclick="window.switchProductDetailTab('movement')">ประวัติการเคลื่อนไหว</button>
        <button type="button" class="product-detail-tab" data-product-tab="all" onclick="window.switchProductDetailTab('all')">ประวัติทั้งหมด</button>
      </div>

      <div id="productDetailPanel_general" class="product-detail-panel">
      ...
      <div id="productDetailPanel_movement" class="product-detail-panel hidden">${movementRows||'<div class="product-empty">ยังไม่มีประวัติรับเข้า–เบิกออก</div>'}</div>
      <div id="productDetailPanel_all" class="product-detail-panel hidden">${historyRows||'<div class="product-empty">ยังไม่มีประวัติสำหรับสินค้านี้</div>'}</div>
    </section>
  </div>`;
  ...
  if(canManageProducts()){
    ...
    if(photoInput) photoInput.onchange=async e=>{
      ...
      try{
        ...
        await updateDoc(productRef(id),{photo:dataUrl,updatedAt:serverTimestamp()});
...
window.saveStockAdjustment=async(id)=>{
  if(!requireManager()) return; const p=state.products.find(x=>x.id===id); if(!p) return;
  const oldStock=Number(p.stock)||0, newStock=Number($('adjustNewStock').value), reason=($('adjustReason').value||'').trim();
  if(!Number.isFinite(newStock)||newStock<0) return toast('ยอดใหม่ต้องเป็น 0 หรือมากกว่า');
  if(newStock===oldStock) return toast('ยอดใหม่เท่ากับยอดเดิม');
  if(!reason) return toast('กรุณาระบุเหตุผล');
  await updateDoc(productRef(id),{stock:newStock,updatedAt:serverTimestamp(),lastAdjustedAt:serverTimestamp(),lastAdjustedBy:state.user.uid,lastAdjustedReason:reason});
  await addLog('ปรับยอดสินค้า',p.name,{productId:id,previousStock:oldStock,newStock,unit:p.unit||'หน่วย',reason,changes:[`จำนวนคงเหลือ: ${oldStock} → ${newStock}`]});
  hideModal(); toast('ปรับยอดและบันทึก Audit Log แล้ว');
};
...
window.saveEditProduct=async(id)=>{
  ...
  await updateDoc(productRef(id),{name,sku,category,unit,min,updatedAt:serverTimestamp()});
...
function renderScan(){
  ...
  view.innerHTML=`<div class="between">
    ...
    <section class="card new-item-form-card">
      ...
      <button type="button" class="new-item-submit ${isOut?'out':'in'}" onclick="window.sendApproval()">
...
window.sendApproval=async()=>{
  ...
  if(!productId) return toast('เลือกสินค้าก่อน');
  if(qty<=0) return toast('กรอกจำนวน');
  if(type==='out'&&!location) return toast('กรุณาเลือกสถานที่เบิก');
  const p=state.products.find(x=>x.id===productId);
  if(type==='out'&&qty>Number(p.stock)) return toast('เบิกเกินสต๊อก');

  const detail=`${type==='out'?'เบิก':'รับ'} ${p.name} ${qty} ${p.unit}${note?` • ${note}`:''}`;
  const logDoc=await addLog('ส่งตรวจ',detail,{
    productId,qty,unit:p.unit,photo:state.selectedImage||'',
    location,moveType:type,note
  });

  await addDoc(userPath('approvals'),{
    productId,name:p.name,qty,unit:p.unit,type,location,
    note,img:state.selectedImage||'',confidence:state.selectedImage?60:0,
    status:'pending',logId:logDoc.id,
    submittedByUid:state.user?.uid||'',
    submittedByName:state.profile?.displayName||state.profile?.username||'',
    submittedByPhoto:state.profile?.photoURL||'',
...
window.approve=async(id)=>{
  ...
  try{
    ...
    if(!a.logId) await addLog('อนุมัติ',`${a.name} ${a.qty} ${a.unit}`,{productId:a.productId,qty:a.qty,unit:a.unit,moveType:a.type,photo:a.img||'',location:a.type==='in'?STORE_LOCATION:(a.location||''),submittedByUid:a.submittedByUid||'',submittedByName:a.submittedByName||'',reviewerUid:state.user.uid,reviewerName:state.profile.displayName||state.profile.username});
    hideModal(); toast('อนุมัติแล้ว');
  }catch(e){ console.error(e); toast(e?.message||'อนุมัติไม่สำเร็จ'); }
};
window.reject=async(id)=>{ if(!requireApprover()) return; const a=state.approvals.find(x=>x.id===id); await deleteDoc(approvalRef(id)); const reviewerUid=state.user?.uid||''; const reviewerName=state.profile?.displayName||state.profile?.username||'ไม่ทราบผู้ใช้'; if(a.logId){ await updateDoc(logDocRef(a.logId),{action:'ปฏิเสธ',time:new Date().toLocaleString('th-TH'),location:a.type==='in'?STORE_LOCATION:(a.location||''),reviewerUid,reviewerName}); } else { await addLog('ปฏิเสธ',a.name,{productId:a.productId,moveType:a.type,location:a.type==='in'?STORE_LOCATION:(a.location||''),submittedByUid:a.submittedByUid||'',submittedByName:a.submittedByName||'',reviewerUid,reviewerName}); } hideModal(); toast('ปฏิเสธรายการแล้ว'); };
...
window.cancelApproval=async(id)=>{
  ...
  try{
    ...
    batch.delete(approvalRef(id));
    if(a.logId){
      batch.update(logDocRef(a.logId),{
        action:'ยกเลิก',
        detail:`ยกเลิก${a.type==='out'?'เบิก':'รับ'} ${a.name} ${a.qty} ${a.unit}`,
    ...
    await batch.commit();
    if(!a.logId){
      await addLog('ยกเลิก',`${a.name} ${a.qty} ${a.unit}`,{
        productId:a.productId,qty:a.qty,unit:a.unit,
        moveType:a.type,location:a.location||''
      });
    }
    hideModal();
...
window.saveApproval=async(id)=>{
  ...
  await updateDoc(approvalRef(id),{
    qty,type,location,
    updatedAt:serverTimestamp(),
    updatedByUid:state.user.uid,
    updatedByName:state.profile?.displayName||state.profile?.username||''
  });

  if(a.logId){
    await updateDoc(logDocRef(a.logId),{
      action:'ส่งตรวจ',
      detail:`${type==='out'?'เบิก':'รับ'} ${a.name} ${qty} ${a.unit}`,
      qty,unit:a.unit,location,moveType:type,
      time:new Date().toLocaleString('th-TH'),
      updatedAt:serverTimestamp()
    });
  }else{
    await addLog('แก้ไขรายการรออนุมัติ',`${a.name} ${qty} ${a.unit}`,{
      productId:a.productId,qty,unit:a.unit,location,moveType:type
    });
  }
  hideModal();
...
window.exportStockBalanceCSV=()=>{
  ...
  const lines=[headers.map(csvCell).join(','),...rows.map(r=>[r.name,r.category,r.sku,r.stock,r.unit,r.min,r.status].map(csvCell).join(','))];
...
window.printStockBalancePDF=()=>{
  ...
  const body=rows.map((r,i)=>`<tr><td>${i+1}</td><td>${escapeHtml(r.name)}</td><td>${escapeHtml(r.category)}</td><td>${escapeHtml(r.sku)}</td><td style="text-align:right;font-weight:700">${r.stock}</td><td>${escapeHtml(r.unit)}</td><td style="text-align:right">${r.min}</td><td>${escapeHtml(r.status)}</td></tr>`).join('');
  const w=window.open('','_blank');
  if(!w){ alert('กรุณาอนุญาต Pop-up เพื่อสร้าง PDF'); return; }
  w.document.write(`<!doctype html><html lang="th"><head><meta charset="utf-8"><title>TheViewStock_Current_Balance</title><style>body{font-family:Arial,'Noto Sans Thai',sans-serif;padding:24px;color:#111827}h1{margin:0 0 6px}.meta{color:#64748b;margin-bottom:18px}.summary{display:flex;gap:10px;flex-wrap:wrap;margin:14px 0}.box{border:1px solid #cbd5e1;border-radius:10px;padding:9px 14px}table{width:100%;border-collapse:collapse;font-size:11px}th,td{border:1px solid #cbd5e1;padding:6px;text-align:left}th{background:#eff6ff}@media print{body{padding:0}.no-print{display:none}}</style></head><body><button class="no-print" onclick="window.print()" style="float:right;padding:10px 16px">พิมพ์ / บันทึกเป็น PDF</button><h1>TheView Stock — ยอดคงเหลือปัจจุบัน</h1><div class="meta">หมวดหมู่: ${escapeHtml(category)} • สร้างเมื่อ ${new Date().toLocaleString('th-TH')}</div><div class="summary"><div class="box">สินค้าทั้งหมด <b>${rows.length}</b></div><div class="box">ปกติ <b>${ok}</b></div><div class="box">ใกล้หมด <b>${low}</b></div><div class="box">หมด <b>${out}</b></div></div><table><thead><tr><th>#</th><th>สินค้า</th><th>หมวดหมู่</th><th>SKU</th><th>คงเหลือ</th><th>หน่วย</th><th>จุดเตือน</th><th>สถานะ</th></tr></thead><tbody>${body}</tbody></table></body></html>`);
  w.document.close();
};
...
function getCurrentReportRows(){
  ...
  return filtered.map(l=>{
    ...
    return {
      ...
      location:l.location||'',
      user:l.userName||l.displayName||l.byName||l.createdByName||'',
      status:l.status||l.action||''
    };
...
window.exportReportCSV=()=>{
  ...
  const headers=['วันที่','เวลา','ประเภท','สินค้า','จำนวน','หน่วย','สถานที่','ผู้ดำเนินการ','สถานะ'];
  const lines=[headers.map(csvCell).join(','),...rows.map(r=>[r.date,r.time,r.type,r.product,r.qty,r.unit,r.location,r.user,r.status].map(csvCell).join(','))];
  const blob=new Blob(['\ufeff'+lines.join('\r\n')],{type:'text/csv;charset=utf-8;'});
  const url=URL.createObjectURL(blob), a=document.createElement('a');
...
window.printReportPDF=()=>{
  ...
  const body=rows.map(r=>`<tr><td>${escapeHtml(r.date)}</td><td>${escapeHtml(r.time)}</td><td>${escapeHtml(r.type)}</td><td>${escapeHtml(r.product)}</td><td style="text-align:right">${r.qty}</td><td>${escapeHtml(r.unit)}</td><td>${escapeHtml(r.location)}</td><td>${escapeHtml(r.user)}</td></tr>`).join('');
  const w=window.open('','_blank');
  if(!w){ alert('กรุณาอนุญาต Pop-up เพื่อสร้าง PDF'); return; }
  w.document.write(`<!doctype html><html lang="th"><head><meta charset="utf-8"><title>${reportFileBase()}</title><style>body{font-family:Arial,'Noto Sans Thai',sans-serif;padding:24px;color:#111827}h1{margin:0 0 6px}.meta{color:#64748b;margin-bottom:18px}.summary{display:flex;gap:12px;margin:14px 0}.box{border:1px solid #cbd5e1;border-radius:10px;padding:10px 16px}table{width:100%;border-collapse:collapse;font-size:12px}th,td{border:1px solid #cbd5e1;padding:7px;text-align:left}th{background:#eff6ff}@media print{body{padding:0}.no-print{display:none}}</style></head><body><button class="no-print" onclick="window.print()" style="float:right;padding:10px 16px">พิมพ์ / บันทึกเป็น PDF</button><h1>TheView Stock</h1><div class="meta">รายงานสต๊อก ${escapeHtml(period)} • สร้างเมื่อ ${new Date().toLocaleString('th-TH')}</div><div class="summary"><div class="box">ทั้งหมด <b>${rows.length}</b> รายการ</div><div class="box">รับเข้า <b>${totalIn}</b></div><div class="box">เบิกออก <b>${totalOut}</b></div></div><table><thead><tr><th>วันที่</th><th>เวลา</th><th>ประเภท</th><th>สินค้า</th><th>จำนวน</th><th>หน่วย</th><th>สถานที่</th><th>ผู้ดำเนินการ</th></tr></thead><tbody>${body}</tbody></table></body></html>`);
  w.document.close();
};
...
window.manageApprovalAssistants=async()=>{ if(!canAssignApprovers()) return toast('เฉพาะกัปตัน/ธุรการ ผู้จัดการ หรือแอดมินเท่านั้น');
  ...
  const rows=staff.map(m=>`<div class="card" style="box-shadow:none;border:1px solid var(--line);margin:10px 0">
    <b>${escapeHtml(m.displayName||m.username)}</b><div class="muted">@${escapeHtml(m.username||'')} • พนักงาน</div>
    <label style="display:flex;gap:10px;align-items:center;margin-top:12px"><input type="checkbox" id="permApprove_${m.uid}" ${m.permissions?.canApprove?'checked':''}> อนุมัติ / ปฏิเสธรายการ</label>
    <label style="display:flex;gap:10px;align-items:center;margin-top:10px"><input type="checkbox" id="permAdjust_${m.uid}" ${m.permissions?.canAdjustStock?'checked':''}> ปรับยอดสต๊อก</label>
    <label style="display:flex;gap:10px;align-items:center;margin-top:10px"><input type="checkbox" id="permProducts_${m.uid}" ${m.permissions?.canManageProducts?'checked':''}> จัดการสินค้า (เพิ่ม/แก้ไข/ลบ)</label>
    <label style="display:flex;gap:10px;align-items:center;margin-top:10px"><input type="checkbox" id="permReports_${m.uid}" ${m.permissions?.canViewReports?'checked':''}> ดูรายงานทั้งหมด</label>
  </div>`).join('');
  openModal('จัดการสิทธิ์พนักงาน',`<p class="note">มอบสิทธิ์เพิ่มเติมเป็นรายบุคคล โดยไม่ต้องเปลี่ยนตำแหน่งจากพนักงาน ระบบจะบันทึกผู้ที่แก้ไขสิทธิ์และเวลาไว้</p>${rows||'<p class="muted">ยังไม่มีพนักงานที่พร้อมกำหนดสิทธิ์</p>'}<button class="btn primary full" onclick="window.saveApprovalAssistants()">บันทึกสิทธิ์</button>`);
};
window.saveApprovalAssistants=async()=>{ if(!canAssignApprovers()) return;
  const staff=state.members.filter(m=>m.role==='staff' && m.status!=='disabled');
  const batch=writeBatch(fs); const changed=[];
  staff.forEach(m=>{
    const permissions={
      canApprove:!!$(`permApprove_${m.uid}`)?.checked,
      canAdjustStock:!!$(`permAdjust_${m.uid}`)?.checked,
      canManageProducts:!!$(`permProducts_${m.uid}`)?.checked,
      canViewReports:!!$(`permReports_${m.uid}`)?.checked
    };
    batch.update(memberRef(m.uid),{permissions,approvalAssignedBy:state.user.uid,approvalAssignedAt:serverTimestamp()});
    changed.push(`${m.displayName||m.username}: ${Object.entries(permissions).filter(([,v])=>v).map(([k])=>k).join(', ')||'ไม่มีสิทธิ์พิเศษ'}`);
  });
  try{
    await batch.commit();
    await addLog('แก้ไขสิทธิ์พนักงาน',changed.join(' | '));
    hideModal(); toast('บันทึกสิทธิ์พนักงานแล้ว');
  }catch(e){ console.error(e); toast(`บันทึกไม่สำเร็จ (${e?.code||'unknown'})`); }
};
window.exportBackup=()=>{
