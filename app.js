...
import { getFirestore, collection, doc, getDoc, setDoc, updateDoc, deleteDoc, onSnapshot, addDoc, serverTimestamp, query, orderBy, getDocs, writeBatch } from 'https://www.gstatic.com/firebasejs/10.12.5/firebase-firestore.js';
...
const LEGACY_OWNER_UIDS = ['KJbRaEl9oKf2uR3N9d84HRXFAHw2'];
const REMEMBER_KEY='theview_remember_username';

function normalizeUsername(value=''){
  return value.trim().toLowerCase().replace(/\s+/g,'');
}
function usernameToEmail(username){
  return `${normalizeUsername(username)}@theview.local`;
}
function loadRememberedLogin(){
  try{
    const username=localStorage.getItem(REMEMBER_KEY)||'';
    if(username){ $('username').value=username; $('rememberMe').checked=true; }
  }catch(e){}
}
function saveRememberedLogin(username){
  try{ localStorage.setItem(REMEMBER_KEY, normalizeUsername(username)); }catch(e){}
}
...
function showScreen(name){
  ...
  $('passwordGate').classList.toggle('hidden',name!=='password');
  $('app').classList.toggle('hidden',name!=='app');
}
function memberRef(uid=state.user?.uid){ return doc(fs,'members',uid); }
function userPath(name){ return collection(fs,'theviewWorkspaces',WORKSPACE_ID,name); }
function productRef(id){ return doc(fs,'theviewWorkspaces',WORKSPACE_ID,'products',id); }
function approvalRef(id){ return doc(fs,'theviewWorkspaces',WORKSPACE_ID,'approvals',id); }
function logDocRef(id){ return doc(fs,'theviewWorkspaces',WORKSPACE_ID,'logs',id); }
function logRef(){ return collection(fs,'theviewWorkspaces',WORKSPACE_ID,'logs'); }
function escapeHtml(s=''){ return String(s).replace(/[&<>\"]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[m])); }
async function addLog(action,detail,extra={}){
  const actor=state.member||{};
  return addDoc(logRef(),{
    action,detail,time:new Date().toLocaleString('th-TH'),createdAt:serverTimestamp(),
    actorUid:state.user?.uid||'',actorUsername:actor.username||'',actorName:actor.displayName||state.user?.email||'',
    ...extra
  });
}
...
async function migrateLegacyDataIfNeeded(){
  if(state.member?.role!=='admin') return {migrated:0};
  const currentProducts=await getDocs(userPath('products'));
  if(!currentProducts.empty) return {migrated:0};
  let migrated=0;
  for(const legacyUid of LEGACY_OWNER_UIDS){
    for(const name of ['products','approvals','logs']){
      const legacy=await getDocs(collection(fs,'theviewUsers',legacyUid,name));
      for(const d of legacy.docs){
        await setDoc(doc(fs,'theviewWorkspaces',WORKSPACE_ID,name,d.id),d.data(),{merge:true});
        migrated++;
      }
    }
  }
  if(migrated){
    await addLog('ย้ายข้อมูลเดิม',`กู้คืนข้อมูลเดิมเข้าสู่พื้นที่ส่วนกลาง ${migrated} รายการ`,{migratedCount:migrated});
  }
  return {migrated};
}
...
async function bootstrapApp(){
  showScreen('app');
  $('memberName').textContent=state.member?.displayName||state.member?.username||'';
  try{
    const result=await migrateLegacyDataIfNeeded();
    bindRealtime();
    render();
    if(result.migrated) toast(`กู้คืนข้อมูลเดิมแล้ว ${result.migrated} รายการ`);
  }catch(e){
    console.error(e);
    bindRealtime();
    render();
    toast('เปิดระบบได้ แต่ย้ายข้อมูลเดิมไม่สำเร็จ');
  }
}
...
// ---------- Realtime listeners: ข้อมูลส่วนกลางของทีม ----------
let unsubProducts=null, unsubApprovals=null, unsubLogs=null;
function bindRealtime(){
  unbindRealtime();
  const onError=(e)=>{ console.error(e); toast('โหลดข้อมูลไม่สำเร็จ: '+(e.code||e.message)); };
  unsubProducts = onSnapshot(userPath('products'), snap=>{ state.products=snap.docs.map(d=>({id:d.id,...d.data()})); render(); },onError);
  unsubApprovals = onSnapshot(userPath('approvals'), snap=>{ state.approvals=snap.docs.map(d=>({id:d.id,...d.data()})); render(); },onError);
  unsubLogs = onSnapshot(query(userPath('logs'), orderBy('createdAt','desc')), snap=>{ state.logs=snap.docs.map(d=>({id:d.id,...d.data()})); render(); },onError);
}
...
$('loginBtn').onclick=async()=>{
  ...
  if(!username||!password) return toast('กรอก Username และรหัสผ่าน');
  $('loginBtn').disabled=true;
  $('loginBtn').textContent='กำลังเข้าสู่ระบบ...';
  state.loginPassword=password;
  try{
    await signInWithEmailAndPassword(auth,usernameToEmail(username),password);
    if($('rememberMe').checked) saveRememberedLogin(username); else clearRememberedLogin();
  }catch(e){
    state.loginPassword='';
    toast('Username หรือรหัสผ่านไม่ถูกต้อง');
  }finally{
...
$('gateLogoutBtn').onclick=()=>signOut(auth);
...
$('changePasswordBtn').onclick=async()=>{
  ...
  if(next.length<6) return toast('รหัสผ่านใหม่ต้องอย่างน้อย 6 ตัว');
  if(next!==confirmNext) return toast('ยืนยันรหัสผ่านไม่ตรงกัน');
  const btn=$('changePasswordBtn');
  btn.disabled=true; btn.textContent='กำลังบันทึก...';
  try{
    if(state.loginPassword){
      const cred=EmailAuthProvider.credential(state.user.email,state.loginPassword);
      await reauthenticateWithCredential(state.user,cred);
    }
    await updatePassword(state.user,next);
    await updateDoc(memberRef(),{mustChangePassword:false,passwordChangedAt:serverTimestamp(),lastLoginAt:serverTimestamp()});
    ...
    console.error(e);
    toast('เปลี่ยนรหัสผ่านไม่สำเร็จ กรุณาออกแล้วเข้าใหม่');
  }finally{ btn.disabled=false; btn.textContent='ตั้งรหัสผ่านใหม่'; }
};
...
onAuthStateChanged(auth, async user=>{
  ...
  try{
    const member=await loadMemberProfile(user);
    if(!member){
      showScreen('login');
      await signOut(auth);
      toast('ไม่พบข้อมูลสมาชิก กรุณาติดต่อผู้ดูแล');
      return;
    }
    if(member.status!=='active'){
      showScreen('login');
      await signOut(auth);
      toast('บัญชีนี้ถูกปิดการใช้งาน');
      return;
    }
    state.member=member;
    if(member.mustChangePassword){
      $('gateMemberName').textContent=member.displayName||member.username||'';
      showScreen('password');
    }else{
      await bootstrapApp();
    }
    ...
    toast('เริ่มระบบไม่สำเร็จ: '+(e.code||e.message));
  }
});
...
function renderProfile(){
  ...
  const auditRows = state.logs.map(l=>{
    const {label,cls} = logPillInfo(l);
    return `<div class="log">
      <span class="pill ${cls}">${escapeHtml(label)}</span>
      <div class="muted" style="margin-top:4px">${escapeHtml(l.time||'')} — ${escapeHtml(l.detail||'')}</div>
      ${l.location?`<div class="muted" style="font-size:13px">📍 ${escapeHtml(l.location)}</div>`:''}
      ${Array.isArray(l.changes)&&l.changes.length?`<ul style="margin:4px 0 0;padding-left:18px">${l.changes.map(c=>`<li class="muted" style="font-size:13px">${escapeHtml(c)}</li>`).join('')}</ul>`:''}
    </div>`;
  }).join('') || '<p class="muted">ยังไม่มี Log</p>';
  view.innerHTML = `<h1>โปรไฟล์</h1>
  <div class="card"><p><b>${escapeHtml(state.member?.displayName||state.member?.username||'')}</b></p><p class="muted">Username: ${escapeHtml(state.member?.username||'')} • ${escapeHtml(state.member?.role||'staff')}</p><button class="btn full" onclick="window.exportBackup()">Export Backup JSON</button></div>
  <div class="card"><button class="btn light full" onclick="window.viewTrash()">🗑️ ถังขยะ${trashCount?` (${trashCount})`:''}</button></div>
  <div class="card"><p class="muted" style="margin-top:0">⚠️ การล้างข้อมูลจะลบสินค้า ประวัติ และรายการทั้งหมดถาวร กู้คืนไม่ได้</p><button class="btn red full" onclick="window.resetAccount()">ล้างข้อมูลส่วนกลางของทีม</button></div>
...
window.resetAccount=async()=>{ const typed=prompt('การล้างข้อมูลจะลบสินค้า ประวัติ และรายการทั้งหมดถาวร กู้คืนไม่ได้\n\nพิมพ์คำว่า "ลบทั้งหมด" เพื่อยืนยัน'); if(typed===null) return; if(typed.trim()!=='ลบทั้งหมด'){ toast('ยกเลิก: ข้อความไม่ตรง ไม่ได้ลบข้อมูล'); return; } const batch=writeBatch(fs); for(const c of ['products','approvals','logs']){ const snap=await getDocs(userPath(c)); snap.docs.forEach(d=>batch.delete(d.ref)); } await batch.commit(); toast('ล้างข้อมูลแล้ว'); };
