...
function renderApproval(){ const canReview=isManager(); view.innerHTML=`<h1>Approval</h1>${!canReview?'<div class="card note">คุณสามารถดูคิวตรวจได้ แต่การอนุมัติ แก้ไข หรือปฏิเสธทำได้โดย Manager/Admin</div>':''}${state.approvals.map(a=>`<div class="card"><div class="between"><div><h2>${escapeHtml(a.name)}</h2><p class="muted"><span class="pill ${a.type==='out'?'warn':'ok'}">${a.type==='out'?'↑ เบิกออก':'↓ รับเข้า'}</span> ${a.qty} ${escapeHtml(a.unit||'')}</p>${a.location?`<p class="muted" style="font-size:13px;margin:2px 0 0">📍 ${escapeHtml(a.location)}</p>`:''}</div><span class="pill warn">รอตรวจ</span></div>${a.img?`<img class="preview" src="${a.img}">`:''}${canReview?`<div class="row"><button class="btn green" onclick="window.confirmApprove('${a.id}')">อนุมัติ</button><button class="btn" onclick="window.editApproval('${a.id}')">แก้ไข</button><button class="btn red" onclick="window.confirmReject('${a.id}')">ปฏิเสธ</button></div>`:''}</div>`).join('')||'<div class="card" style="text-align:center"><p style="font-size:40px;margin:0 0 6px">✅</p><p class="muted" style="margin:0">ไม่มีงานค้าง ทุกอย่างเรียบร้อย</p></div>'}`; }
