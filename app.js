...
const state = { user:null, profile:null, members:[], page:VALID_PAGES.has(savedPage)?savedPage:'home', products:[], approvals:[], logs:[], auditLogs:[], selectedImage:null, imageMode:null, viewProductId:null, tempMoveImage:null, tempProductImage:null, stockFilter:'all', stockSearch:'', stockSort:'name-asc', reportMode:'day', reportFilter:'all', reportDate:'', reportMonth:'', reportStart:'', reportEnd:'', historySearch:'', historyFilter:'all', historyStart:toDateStr(new Date()), historyEnd:toDateStr(new Date()) };
try{
  const savedUi=JSON.parse(localStorage.getItem(UI_STATE_KEY)||'{}');
  ['stockFilter','stockSort','reportMode','reportFilter','reportDate','reportMonth','reportStart','reportEnd','historySearch','historyFilter','historyStart','historyEnd','newItemType'].forEach(k=>{ if(savedUi[k]!==undefined) state[k]=savedUi[k]; });
}catch(_){ }
function saveUiState(){
  const keys=['stockFilter','stockSort','reportMode','reportFilter','reportDate','reportMonth','reportStart','reportEnd','historySearch','historyFilter','historyStart','historyEnd','newItemType'];
