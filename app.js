...
const PRODUCT_CACHE_DB='theview-stock-cache-v1';
const PRODUCT_CACHE_STORE='kv';
const PRODUCT_CACHE_KEY='products-main';
...
window.saveEditProduct=async(id)=>{
  ...
  const p=state.products.find(x=>x.id===id);
  if(!p) return toast('ไม่พบสินค้านี้ กรุณารีเฟรชแล้วลองใหม่');
  try{
    await updateDoc(productRef(id),{trashed:true,trashedAt:serverTimestamp(),updatedAt:serverTimestamp()});
    try{ await addLog('ย้ายไปถังขยะ',p.name,{productId:id}); }catch(logErr){ console.warn('บันทึก Log หลังย้ายถังขยะไม่สำเร็จ',logErr); }
    toast('ย้ายไปถังขยะแล้ว กู้คืนได้ในโปรไฟล์');
    hideModal();
  }catch(e){
    console.error('ย้ายสินค้าไปถังขยะไม่สำเร็จ',e);
    const msg=e?.code==='permission-denied'
      ? `ย้าย “${p.name||'สินค้า'}” ไม่สำเร็จ: สิทธิ์ของรายการนี้ไม่ตรงกับระบบปัจจุบัน`
      : `ย้าย “${p.name||'สินค้า'}” ไม่สำเร็จ กรุณาลองใหม่`;
    toast(msg);
  }
};
...
window.exportBackup=()=>{
  ...
  const data={
    ...
    sourceStorageBucket:firebaseConfig.storageBucket||'',
    sourceEnvironment:(firebaseConfig.projectId==='wjb-story'?'test':'production'),
    products:clean(state.products),
    approvals:clean(state.approvals),
    logs:clean(state.logs),
    auditLogs:clean(state.auditLogs),
    exportedAt:new Date().toISOString(),
    exportedBy:state.profile?.displayName||state.profile?.username||''
  };
  const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'});
  const a=document.createElement('a'),url=URL.createObjectURL(blob);
  a.href=url; a.download=`theview-backup-${data.sourceEnvironment}-${toDateStr(new Date())}.json`; a.click();
  setTimeout(()=>URL.revokeObjectURL(url),1000);
};
...
async function commitInChunks(ops){
  for(let i=0;i<ops.length;i+=350){
    const batch=writeBatch(fs);
    ops.slice(i,i+350).forEach(op=>batch.set(op.ref,op.data,op.options||{merge:true}));
    await batch.commit();
  }
}
function sanitizeImportedProduct(rest,crossProject){
  const allowed=['name','sku','skuKey','category','unit','stock','min','archived','trashed','trashedAt','photo','photoPath'];
  const out={}; allowed.forEach(k=>{ if(rest[k]!==undefined) out[k]=rest[k]; });
  out.name=String(out.name||'').trim();
  out.sku=String(out.sku||'').trim();
  out.skuKey=normalizeSkuKey(out.sku);
  out.stock=Math.max(0,Number(out.stock)||0);
  out.min=Math.max(0,Number(out.min)||0);
  out.archived=!!out.archived; out.trashed=!!out.trashed;
  // รูป URL จากอีก Firebase project ไม่ควรถูกผูกข้าม environment
  if(crossProject && isStorageUrl(out.photo)){ out.photo=''; out.photoPath=''; out.importImageNeedsUpload=true; }
  return out;
}
$('backupInput')?.addEventListener('change',async e=>{
  ...
  try{
    ...
    const sourceProjectId=String(data?.sourceProjectId||'').trim();
    const currentProjectId=String(firebaseConfig.projectId||'').trim();
    const crossProject=!!sourceProjectId && sourceProjectId!==currentProjectId;
    const legacyUnknown=!sourceProjectId;
    const productCount=Array.isArray(data.products)?data.products.length:0;
    const otherCount=['approvals','logs','auditLogs'].reduce((n,g)=>n+(Array.isArray(data[g])?data[g].length:0),0);
    let promptText=`นำเข้า Backup ${productCount} สินค้า`;
    if(crossProject){
      promptText+=`\n\nตรวจพบว่าไฟล์มาจาก Firebase คนละโปรเจกต์ (${sourceProjectId} → ${currentProjectId})`+
        `\nระบบจะนำเข้า “สินค้า” แบบแยก environment เท่านั้น`+
        `\n• ไม่ย้าย UID/เจ้าของ/สิทธิ์จากระบบต้นทาง`+
        `\n• ไม่ย้าย Audit Log/Approval/Logs เดิมข้ามโปรเจกต์`+
        `\n• รูปจาก Storage ต้นทางจะไม่ผูกข้ามระบบ ต้องอัปโหลดรูปใหม่ในปลายทาง`;
    }else if(legacyUnknown){
      promptText+=`\n\nไฟล์ Backup รุ่นเก่าไม่มี sourceProjectId ระบบจะนำเข้าอย่างระมัดระวัง`;
    }else{
      promptText+=`\nพร้อมรวมข้อมูลกับโปรเจกต์ปัจจุบัน`;
    }
    if(otherCount && crossProject) promptText+=`\n\nข้อมูลธุรกรรมเดิม ${otherCount} รายการจะถูกข้ามเพื่อป้องกัน UID ข้ามระบบ`;
    ...
    if(Array.isArray(data.products)){
      for(const item of data.products){
        const {id,...rest}=item||{}; if(!id) continue;
        const product=sanitizeImportedProduct(rest,crossProject);
        if(!product.name) continue;
        const payload={...product,createdAt:serverTimestamp(),updatedAt:serverTimestamp(),importedAt:serverTimestamp(),importedBy:state.user.uid,importedByName:state.profile?.displayName||state.profile?.username||'',importedSourceProjectId:sourceProjectId||'legacy-unknown'};
        ops.push({ref:doc(fs,'theviewWorkspaces','main','products',id),data:payload,options:{merge:true}});
        importedProducts.push({id,...product});
        if(product.sku){
          ops.push({ref:skuRegistryDocRef(product.sku),data:{sku:product.sku,skuKey:normalizeSkuKey(product.sku),productId:id,updatedAt:serverTimestamp(),importedAt:serverTimestamp()},options:{merge:true}});
        }
      }
    }

    // ข้อมูลใน project เดียวกันเท่านั้นจึงคงธุรกรรมเดิมไว้
    if(!crossProject){
      for(const group of ['approvals','logs']){
        if(!Array.isArray(data[group])) continue;
        for(const item of data[group]){
          const {id,...rest}=item||{}; if(!id) continue;
          delete rest.createdAt;
          ops.push({ref:doc(fs,'theviewWorkspaces','main',group,id),data:{...rest,createdAt:serverTimestamp(),importedAt:serverTimestamp(),importedBy:state.user.uid},options:{merge:true}});
        }
      }
      // auditLogs เป็น immutable: ไม่ import ทับย้อนหลัง เพื่อไม่ให้ติด UID จากไฟล์หรือชน Security Rules
    }

    if(!ops.length) throw new Error('ไม่พบข้อมูลที่รองรับในไฟล์');
    await commitInChunks(ops);
    try{ await addLog('นำเข้า Backup',`${importedProducts.length} สินค้า${crossProject?' (ข้ามโปรเจกต์แบบปลอดภัย)':''}`); }catch(_){}
    try{ await addAudit('นำเข้า Backup',`${importedProducts.length} สินค้า • source=${sourceProjectId||'legacy-unknown'} • target=${currentProjectId}`); }catch(_){}
    toast(`นำเข้าสำเร็จ ${importedProducts.length} สินค้า${crossProject?' • แยกข้อมูลข้ามโปรเจกต์แล้ว':''}`);
