...
import { getAuth, onAuthStateChanged, signInWithEmailAndPassword, createUserWithEmailAndPassword, signOut, setPersistence, browserLocalPersistence, updatePassword } from 'https://www.gstatic.com/firebasejs/10.12.5/firebase-auth.js';
import { getFirestore, collection, doc, setDoc, updateDoc, deleteDoc, onSnapshot, addDoc, serverTimestamp, query, orderBy, getDocs, getDoc, writeBatch } from 'https://www.gstatic.com/firebasejs/10.12.5/firebase-firestore.js';
...
function makeUsername(first,last){ return normalizeUsername(first) + normalizeUsername(last).charAt(0); }
...
onAuthStateChanged(auth, async user=>{
  ...
  if(user){
    let snap=await getDoc(memberRef(user.uid));
    if(!snap.exists()){
      const username=(user.email||'admin').split('@')[0];
      await setDoc(memberRef(user.uid),{username,firstName:'System',lastName:'Admin',displayName:'System Admin',role:'admin',status:'active',mustChangePassword:false,createdAt:serverTimestamp()});
      snap=await getDoc(memberRef(user.uid));
    }
    state.profile={uid:user.uid,...snap.data()};
    if(state.profile.status==='disabled'){ await signOut(auth); return toast('บัญชีนี้ถูกปิดใช้งาน'); }
    await seedIfEmpty(); bindRealtime(); render();
    if(state.profile.mustChangePassword) setTimeout(()=>window.openFirstPasswordChange(),250);
  } else{ unbindRealtime(); }
});
...
function render(){ if(!state.user) return; ({home:renderHome,stock:renderStock,scan:renderScan,approval:renderApproval,profile:renderProfile,trash:renderTrash,report:renderReport,productDetail:()=>renderProductDetail(state.viewProductId)}[state.page]||renderHome)(); }
...
function renderStock(){
  ...
  view.innerHTML = `<div class="between"><h1>Stock${filterLow?' • ใกล้หมด':''}</h1><button class="btn primary small" onclick="window.addProduct()">+ เพิ่ม</button></div>${filterLow?`<div class="card" style="display:flex;justify-content:space-between;align-items:center;gap:10px"><span class="muted">กำลังแสดงเฉพาะสินค้าใกล้หมด</span><button class="btn small" onclick="window.goToPage('stock')">แสดงทั้งหมด</button></div>`:''}<div class="card">${rows||emptyMsg}</div>${archivedCount?`<div class="card"><button class="btn light full" onclick="window.showArchived()">📦 ดูรายการที่ Archive แล้ว (${archivedCount})</button></div>`:''}`;
...
window.addProduct=()=>openModal('เพิ่มสินค้า',`
...
window.editProduct=(id)=>{ const p=state.products.find(x=>x.id===id); openModal('แก้ไขสินค้า',`
  <label class="field-label" for="pn">ชื่อสินค้า</label>
...
window.editApproval=(id)=>{ const a=state.approvals.find(x=>x.id===id); openModal('แก้ไขรายการ',`<input id="aq" type="number" value="${a.qty}"><select id="at"><option value="out" ${a.type==='out'?'selected':''}>เบิกออก</option><option value="in" ${a.type==='in'?'selected':''}>รับเข้า</option></select>${locationFieldHtml('aLoc','aLocOther')}<button class="btn primary full" onclick="window.saveApproval('${id}')">บันทึก</button>`); const sel=$('aLoc'); if(sel && a.location){ if(LOCATION_OPTIONS.includes(a.location)){ sel.value=a.location; } else { sel.value='อื่นๆ'; $('aLocOther').classList.remove('hidden'); $('aLocOther').value=a.location; } } };
window.saveApproval=async(id)=>{ const qty=Number($('aq').value)||0; if(qty<=0) return toast('จำนวนไม่ถูกต้อง'); const type=$('at').value; const location=getLocationValue('aLoc','aLocOther'); await updateDoc(approvalRef(id),{qty,type,location}); await addLog('แก้ไขรายการรอตรวจ',state.approvals.find(x=>x.id===id)?.name||'',{productId:state.approvals.find(x=>x.id===id)?.productId,location}); closeModal(); };
...
window.openChangePassword=()=>openModal('เปลี่ยนรหัสผ่าน',`<input id="newPass" type="password" placeholder="รหัสผ่านใหม่ อย่างน้อย 6 ตัว"><input id="confirmPass" type="password" placeholder="ยืนยันรหัสผ่านใหม่"><button class="btn primary full" onclick="window.saveNewPassword(false)">บันทึก</button>`);
window.openFirstPasswordChange=()=>{ $('closeModal').classList.add('hidden'); openModal('ตั้งรหัสผ่านใหม่ครั้งแรก',`<p class="muted">เพื่อความปลอดภัย กรุณาเปลี่ยนรหัสผ่านเริ่มต้นก่อนใช้งาน</p><input id="newPass" type="password" placeholder="รหัสผ่านใหม่ อย่างน้อย 6 ตัว"><input id="confirmPass" type="password" placeholder="ยืนยันรหัสผ่านใหม่"><button class="btn primary full" onclick="window.saveNewPassword(true)">ตั้งรหัสผ่านใหม่</button>`); };
window.saveNewPassword=async(first)=>{ const a=$('newPass').value,b=$('confirmPass').value; if(a.length<6)return toast('รหัสผ่านอย่างน้อย 6 ตัว'); if(a!==b)return toast('รหัสผ่านไม่ตรงกัน'); if(a===DEFAULT_PASSWORD)return toast('กรุณาตั้งรหัสผ่านอื่น'); try{ await updatePassword(auth.currentUser,a); await updateDoc(memberRef(),{mustChangePassword:false,passwordChangedAt:serverTimestamp()}); state.profile.mustChangePassword=false; $('closeModal').classList.remove('hidden'); closeModal(); toast('เปลี่ยนรหัสผ่านแล้ว'); }catch(e){ toast('เปลี่ยนรหัสผ่านไม่สำเร็จ กรุณาออกแล้วเข้าใหม่'); } };
window.manageMembers=async()=>{ const snap=await getDocs(collection(fs,'members')); state.members=snap.docs.map(d=>({uid:d.id,...d.data()})); const rows=state.members.map(m=>`<div class="product"><div><b>${escapeHtml(m.displayName||m.username)}</b><div class="muted">${escapeHtml(m.username)} • ${escapeHtml(m.role||'staff')} • ${m.status==='disabled'?'ปิดใช้งาน':'ใช้งาน'}</div></div>${m.uid!==state.user.uid?`<button class="btn small" onclick="window.toggleMember('${m.uid}','${m.status==='disabled'?'active':'disabled'}')">${m.status==='disabled'?'เปิด':'ปิด'}</button>`:''}</div>`).join(''); openModal('จัดการสมาชิก',`<button class="btn primary full" onclick="window.openAddMember()">+ เพิ่มสมาชิก</button><p class="note">Username = ชื่อจริง + ตัวแรกของนามสกุล เช่น Jettaphon Jaioun → jettaphonj</p>${rows}`); };
window.openAddMember=()=>openModal('เพิ่มสมาชิก',`<input id="mf" placeholder="ชื่อจริงภาษาอังกฤษ" oninput="window.previewUsername()"><input id="ml" placeholder="นามสกุลภาษาอังกฤษ" oninput="window.previewUsername()"><input id="mu" placeholder="Username" readonly><select id="mr"><option value="staff">Staff</option><option value="manager">Manager</option><option value="admin">Administrator</option></select><button class="btn primary full" onclick="window.createMember()">สร้างบัญชี</button><p class="note">Password เริ่มต้น: chartered และระบบจะบังคับเปลี่ยนเมื่อเข้าสู่ระบบครั้งแรก</p>`);
window.previewUsername=()=>{ $('mu').value=makeUsername($('mf').value,$('ml').value); };
window.createMember=async()=>{ const first=$('mf').value.trim(),last=$('ml').value.trim(),username=makeUsername(first,last),role=$('mr').value; if(!first||!last||!username)return toast('กรอกชื่อและนามสกุล'); try{ const secondary=initializeApp(firebaseConfig,'member-'+Date.now()); const secondaryAuth=getAuth(secondary); const cred=await createUserWithEmailAndPassword(secondaryAuth,usernameToEmail(username),DEFAULT_PASSWORD); await setDoc(doc(fs,'members',cred.user.uid),{username,firstName:first,lastName:last,displayName:`${first} ${last}`,role,status:'active',mustChangePassword:true,createdAt:serverTimestamp(),createdBy:state.user.uid}); await signOut(secondaryAuth); await addLog('เพิ่มสมาชิก',`${first} ${last} (${username})`); toast('สร้างสมาชิกแล้ว'); closeModal(); }catch(e){ toast(e.code==='auth/email-already-in-use'?'Username นี้มีแล้ว':'สร้างสมาชิกไม่สำเร็จ: '+e.message); } };
window.toggleMember=async(uid,status)=>{ await updateDoc(memberRef(uid),{status}); await addLog(status==='disabled'?'ปิดบัญชีสมาชิก':'เปิดบัญชีสมาชิก',uid); closeModal(); toast('อัปเดตสถานะแล้ว'); };
window.viewTrash()">🗑️ ถังขยะ${trashCount?` (${trashCount})`:''}</button></div>
  <div class="card"><p class="muted" style="margin-top:0">⚠️ การล้างข้อมูลจะลบสินค้า ประวัติ และรายการทั้งหมดถาวร กู้คืนไม่ได้</p><button class="btn red full" onclick="window.resetAccount()">ล้างข้อมูลบัญชีนี้</button></div>
  <div class="card"><h2>Audit Log</h2>${auditRows}</div>`;
}
