...
function renderProductDetail(id){
  ...
  <div class="product-detail-shell">
    <section class="product-detail-card">
      ...
      ${canManageProducts()?`<button type="button" class="product-detail-edit-btn" onclick="window.editProduct('${p.id}')"><span>✏️ แก้ไขข้อมูลสินค้า</span><span>›</span></button>`:''}
...
function renderHistory(){
  ...
  const renderLog=(l)=>{
    ...
    const movementText=l.action==='อนุมัติ' && l.moveType
      ? `<div class="muted" style="font-size:13px;margin-top:4px">${l.moveType==='in'?'📥 อนุมัติรับเข้า':'📤 อนุมัติเบิกออก'}</div>`
      : '';
    ...
    return `<div class="card" style="margin:10px 0">
      <div class="between" style="align-items:flex-start;gap:10px">
        ...
        <div class="muted" style="font-size:12px;text-align:right;white-space:nowrap">${escapeHtml(timeText)}</div>
      </div>
    </div>`;
  };
  const rows=dateKeys.map(key=>`<section class="history-day-group">
    <h2 style="font-size:18px;margin:18px 4px 8px">📅 ${escapeHtml(thaiFullDate(key))} <span class="muted" style="font-size:13px;font-weight:500">(${grouped[key].length} รายการ)</span></h2>
    ${grouped[key].map(renderLog).join('')}
  </section>`).join('');
  const periodText=startDate===endDate ? thaiFullDate(startDate) : `${thaiFullDate(startDate)} ถึง ${thaiFullDate(endDate)}`;
  ...
  view.innerHTML=`<div class="between">
    ...
    <div class="card" style="display:grid;gap:10px">
      <div class="history-date-grid" style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <label><span class="muted" style="font-size:13px">ตั้งแต่วันที่</span><input type="date" value="${escapeHtml(startDate)}" onchange="window.setHistoryDateRange(this.value,null)"></label>
        <label><span class="muted" style="font-size:13px">ถึงวันที่</span><input type="date" value="${escapeHtml(endDate)}" onchange="window.setHistoryDateRange(null,this.value)"></label>
      </div>
      <div class="row" style="gap:8px;flex-wrap:wrap">
        <button class="btn small primary" onclick="window.setHistoryToday()">วันนี้</button>
        <button class="btn small light" onclick="window.shiftHistoryDay(-1)">← วันก่อน</button>
        <button class="btn small light" onclick="window.shiftHistoryDay(1)">วันถัดไป →</button>
      </div>
      ...
      <div class="muted" style="font-size:13px">ช่วงที่เลือก: ${escapeHtml(periodText)} • แสดง ${logs.length} รายการ</div>
    </div>
    <div>${rows||'<div class="card"><p class="muted" style="margin:0">ไม่พบประวัติในวันที่หรือช่วงวันที่เลือก</p></div>'}</div>`;
}
...
window.exportBackup=()=>{
  ...
  const data={version:'28.6',workspace:'main',products:clean(state.products),approvals:clean(state.approvals),logs:clean(state.logs),exportedAt:new Date().toISOString(),exportedBy:state.profile?.displayName||state.profile?.username||''};
