...
function renderHome(){
  ...
  const low=active.filter(p=>Number(p.stock)<=Number(p.min)).length;
  const pending=state.approvals.length;
  const todayStr=toDateStr(new Date());
  const todayLogs=state.logs.filter(l=>{ const d=getLogDate(l); return d && toDateStr(d)===todayStr; });
  ...
  view.innerHTML=`<h1>หน้าแรก</h1>
    ...
    <div class="grid home-dashboard-grid">
      <div class="stat clickable" onclick="window.goToPage('stock')"><span>สินค้า</span><b>${active.length}</b></div>
      <div class="stat clickable" onclick="window.goToPage('stock',{filter:'low'})"><span>ใกล้หมด</span><b>${low}</b></div>
      <div class="stat clickable" onclick="window.goToPage('history')"><span>ประวัติวันนี้</span><b>${todayLogs.length}</b></div>
      ${canViewReports()?`<div class="stat clickable" onclick="window.goToPage('report')"><span>รายงานสต๊อก</span><b>📊</b></div>`:''}
    </div>

    <div class="card home-system-card">
      <h2>สถานะระบบ</h2>
      <button class="home-system-row ${pending?'warn':'ok'}" onclick="window.goToPage('approval')">
        <span>${pending?'🔔':'✅'} ${approvalTitle}</span><b>${pending} รายการ</b>
      </button>
      <button class="home-system-row ${low?'warn':'ok'}" onclick="window.goToPage('stock',{filter:'low'})">
        <span>${low?'⚠️':'✅'} สินค้าใกล้หมด</span><b>${low} รายการ</b>
      </button>
    </div>`;
}
function renderStock(){
  ...
  view.innerHTML = `<div class="between">
      ...
      ${isManager()?'<button class="btn primary small" onclick="window.addProduct()">+ เพิ่ม</button>':''}
    </div>
...
window.editProduct=(id)=>{ if(!requireManager()) return; const p=state.products.find(x=>x.id===id); openModal('แก้ไขสินค้า',`
  ...
  <label class="field-label" for="pstock">จำนวนคงเหลือปัจจุบัน</label>
  <span class="field-hint">แอดมิน ผู้จัดการ และกัปตันสามารถแก้ไขยอดจริงได้ ระบบจะบันทึกประวัติการเปลี่ยนแปลง</span>
  <input id="pstock" type="number" min="0" step="1" value="${Number(p.stock)||0}" placeholder="เช่น 10">

  <label class="field-label" for="padjustNote">เหตุผลที่แก้ไขยอด (บังคับเมื่อยอดเปลี่ยน)</label>
  <input id="padjustNote" value="" placeholder="เช่น ตรวจนับสต๊อกจริง">
...
window.saveEditProduct=async(id)=>{
  ...
  const adjustNote=($('padjustNote').value||'').trim();
  if(!Number.isFinite(stock) || stock<0) return toast('จำนวนคงเหลือต้องเป็น 0 หรือมากกว่า');
  if(stock!==Number(p.stock||0) && !adjustNote) return toast('กรุณาระบุเหตุผลที่แก้ไขยอด');
  const FIELD_LABELS={name:'ชื่อสินค้า',sku:'รหัสสินค้า (SKU)',category:'หมวดหมู่',unit:'หน่วยนับ',stock:'จำนวนคงเหลือ',min:'จุดเตือนสต๊อกต่ำ'};
  const before={name:p.name||'',sku:p.sku||'',category:p.category||'',unit:p.unit||'',stock:Number(p.stock)||0,min:p.min||0};
  const after={name,sku,category,unit,stock,min};
  const changes=[];
  ...
  await updateDoc(productRef(id),{name,sku,category,unit,stock,min,updatedAt:serverTimestamp()});
  const stockChanged=Number(before.stock)!==Number(stock);
  await addLog(stockChanged?'ปรับยอดสินค้า':'แก้ไขสินค้า',name,{productId:id,changes:changes.length?changes:['ไม่มีการเปลี่ยนแปลงข้อมูล'],previousStock:Number(before.stock),newStock:stock,adjustNote});
  hideModal();
};
...
window.exportBackup=()=>{
  ...
  const data={version:'30.0-phase1',workspace:'main',products:clean(state.products),approvals:clean(state.approvals),logs:clean(state.logs),auditLogs:clean(state.auditLogs),exportedAt:new Date().toISOString(),exportedBy:state.profile?.displayName||state.profile?.username||''};
