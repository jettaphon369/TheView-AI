    ...
    ${state.approvals.map(a=>{
      ...
      return `<div class="card approval-card">
        <div class="approval-head">
          ...
          <span class="pill warn approval-status">รออนุมัติ</span>
        </div>

        ${canReview?`<div class="muted" style="font-size:13px;margin:-4px 0 12px">👤 ผู้ส่ง: ${escapeHtml(a.submittedByName||'-')}</div>`:''}
        ...
        </div>`:(own?`<div class="approval-actions">
          <button class="btn primary" onclick="window.editApproval('${a.id}')">✎ แก้ไขรายการ</button>
          <button class="btn red" onclick="window.confirmCancelApproval('${a.id}')">ยกเลิกรายการ</button>
        </div>`:'')}
      </div>`;
...
window.exportBackup=()=>{
  ...
  const data={version:'27.9',workspace:'main',products:clean(state.products),approvals:clean(state.approvals),logs:clean(state.logs),exportedAt:new Date().toISOString(),exportedBy:state.profile?.displayName||state.profile?.username||''};
