...
const BUILD_VERSION='34.21.2-ADMIN-MEMBER-DELETE';
...
// Production guard: remove the legacy TEST product cache that was accidentally reused.
// This only deletes browser-side cached TEST products; it does not delete Firestore data.
try{
  if('indexedDB' in window){
    indexedDB.deleteDatabase('theview-stock-cache-v2-wjb-story');
  }
}catch(_){ /* ignore cache cleanup errors */ }
...
async function bindProductsOptimized(options={}){
  ...
  try{
    ...
    const needsFullSync=forceFullSync || !cacheUsable || !cached?.lastFullSync || (now-cached.lastFullSync)>=PRODUCT_CACHE_TTL_MS;
...
onAuthStateChanged(auth, async user=>{
  ...
  try{
    ...
    // v34.16.2: First Password Gate เป็น Server-side gate
    // ฝั่ง Client อ่านสถานะได้เท่านั้น และไม่มีสิทธิ์ปิด mustChangePassword เอง
    ...
    if(!mustChangePassword && state.profile.passwordChangePending === true){
      // รองรับข้อมูลเก่าที่มี pending ค้าง โดยไม่เขียนทับจาก Client
...
function lotSummaryMarkup(product={}){
  const lots=activeProductLots(product),names=lotDisplayMap(product); if(!lots.length) return '';
  const first=lots[0]; return `<div class="muted" style="font-size:12px;margin-top:4px">📦 ${lots.length} ล็อต • ควรเบิก ${escapeHtml(names.get(first.id)||first.lotNo)}${first.expiryDate?` • หมดอายุ ${escapeHtml(lotDateLabel(first.expiryDate))}`:''}</div>`;
}
...
function productQrPayload(product={}){
  ...
  url.searchParams.set('v','34.21.1');
  return url.toString();
}
...
function renderProductQrLabel(product={}){
  ...
  return `<section class="product-qr-label">
    <div class="product-qr-brand">CHEE CHAN STOCK</div>
...
window.openProductQr=(id)=>{
  ...
  openModal('QR Code สินค้า',`<div class="product-qr-sheet">
    ...
    <div class="product-qr-actions">
      <button class="btn green" onclick="window.printProductQr('${escapeHtml(product.id)}')">🖨️ พิมพ์ป้าย QR</button>
...
function tryOpenProductDeepLink(){
  ...
  const product=state.products.find(p=>p.id===pendingTarget || normalizeSkuKey(p.sku||'')===normalizeSkuKey(pendingTarget));
...
function renderHome(){
  ...
  view.innerHTML=`<div class="dashboard-heading"><div><div class="dashboard-kicker">ภาพรวมคลังสินค้า</div><h1>หน้าแรก</h1></div><div class="dashboard-date">${new Date().toLocaleDateString('th-TH',{day:'numeric',month:'long',year:'numeric'})}</div></div>
    ...
    <div class="dashboard-stat-grid">
      ...
      <button class="dashboard-stat stat-low" onclick="window.goToPage('stock',{filter:'low'})"><span class="dashboard-stat-icon">⚠️</span><div><small>สต๊อกใกล้หมด</small><b>${low}</b><em>${low?'ควรตรวจสอบ':'สถานะปกติ'}</em></div></button>
      <button class="dashboard-stat stat-expiry" onclick="window.goToPage('stock',{filter:'expiry'})"><span class="dashboard-stat-icon">⏰</span><div><small>ใกล้หมดอายุ</small><b>${expiryCount}</b><em>${expiryCount?'ควรตรวจสอบ':'สถานะปกติ'}</em></div></button>
    </div>
    ...
    <section class="card dashboard-alert-card">
      <div class="dashboard-section-head"><div><small>ต้องดูแล</small><h2>แจ้งเตือนสินค้า</h2></div></div>
      <div class="alert-tabs-summary">
        <button onclick="window.goToPage('stock',{filter:'low'})"><span>📦 สต๊อกใกล้หมด</span><b>${low}</b></button>
        <button onclick="window.goToPage('stock',{filter:'expiry'})"><span>⏰ ใกล้หมดอายุ</span><b>${expiryCount}</b></button>
      </div>
      <div class="dashboard-alert-columns">
        <section class="dashboard-alert-group">
          <div class="dashboard-alert-group-head"><b>📦 สินค้าใกล้หมดสต๊อก</b><button onclick="window.goToPage('stock',{filter:'low'})">ดูทั้งหมด ›</button></div>
          <div class="dashboard-low-list">${lowAlertRows}</div>
        </section>
        <section class="dashboard-alert-group">
          <div class="dashboard-alert-group-head"><b>⏰ สินค้าใกล้หมดอายุ</b><button onclick="window.goToPage('stock',{filter:'expiry'})">ดูทั้งหมด ›</button></div>
          <div class="dashboard-low-list">${expiryAlertRows}</div>
        </section>
      </div>
    </section>`;
}
...
function renderStock(){
  ...
  const rows=list.map(p=>{
    ...
    const accent=expiryStatus.key==='expired'?'#111827':(expiryStatus.key==='today'?'#dc2626':(expiryStatus.key==='warning'?'#f59e0b':(isOut?'#ef4444':(isLow?'#f59e0b':'#22c55e'))));
    ...
    return `<article class="stock-card-modern" style="--stock-accent:${accent}" role="button" tabindex="0" aria-label="ดูรายละเอียด ${escapeHtml(p.name)}" onclick="window.viewProduct('${p.id}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();window.viewProduct('${p.id}')}">
      ...
      <div class="stock-card-main">
        ...
        <div class="stock-card-sku">รหัสสินค้า: ${escapeHtml(p.sku||'-')}</div>${lotSummaryMarkup(p)}
        ...
        <div class="stock-card-qty"><span class="stock-card-number">${stock}</span><span class="stock-card-unit">${escapeHtml(p.unit||'หน่วย')}</span></div>
      </div>
  ...
  const emptyMsg=queryText
    ...
    : (filterLow?'<div class="card"><p class="muted">ไม่มีสต๊อกใกล้หมด 🎉</p></div>':(filterExpiry?'<div class="card"><p class="muted">ไม่มีสินค้าใกล้หมดอายุ 🎉</p></div>':'<div class="card"><p class="muted">ยังไม่มีสินค้า</p></div>'));
  const pageSuffix=filterLow?' • สต๊อกใกล้หมด':(filterExpiry?' • ใกล้หมดอายุ':'');
  ...
  view.innerHTML=`<div class="between stock-page-head">
    ...
    ${(filterLow||filterExpiry)?`<div class="card stock-active-filter"><span class="muted">${filterLow?'กำลังแสดงเฉพาะสต๊อกใกล้หมด':'กำลังแสดงเฉพาะสินค้าใกล้หมดอายุ เรียงวันที่ใกล้ที่สุดก่อน'}</span><button class="btn small" onclick="window.goToPage('stock',{filter:'all',resetScroll:true})">แสดงทั้งหมด</button></div>`:''}
...
function refreshStockSearchResults(){
  ...
  if(state.stockCategory!=='all') list=list.filter(p=>String(p.category||'').trim()===state.stockCategory);
  if(queryText){
    list=list.filter(p=>[p.name,p.sku,p.category,p.unit].map(v=>String(v||'').toLowerCase()).join(' ').includes(queryText));
  }
  ...
  const rows=list.map(p=>{
    ...
    return `<article class="stock-card-modern" style="--stock-accent:${accent}" role="button" tabindex="0" aria-label="ดูรายละเอียด ${escapeHtml(p.name)}" onclick="window.viewProduct('${p.id}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();window.viewProduct('${p.id}')}">
      <div class="stock-card-photo">${p.photo?productImageMarkup(p.photo,p.name):`<div class="stock-card-photo-placeholder">📦</div>`}</div>
      <div class="stock-card-main"><h3 class="stock-card-name">${escapeHtml(p.name)}</h3><div class="stock-card-sku">รหัสสินค้า: ${escapeHtml(p.sku||'-')}</div>${lotSummaryMarkup(p)}${expiryBadge?`<div class="stock-card-expiry-row">${expiryBadge}<span>หมดอายุ ${escapeHtml(expiryDateLabel(p))}</span></div>`:''}<div class="stock-card-label">คงเหลือ</div><div class="stock-card-qty"><span class="stock-card-number">${stock}</span><span class="stock-card-unit">${escapeHtml(p.unit||'หน่วย')}</span></div></div>
      <div class="stock-card-side"><span class="stock-status-modern ${statusClass}">${statusText}</span><span class="stock-card-arrow">›</span></div>
    </article>`;
  }).join('');
...
window.saveNewProduct=async()=>{
  ...
  const product={name,sku,skuKey:normalizeSkuKey(sku),category:$('pc').value,unit:$('pu').value||'ชิ้น',stock:initialStock,min:minimumStock,...expiry,lots:initialLot?[initialLot]:[],archived:false,photo:'',photoPath:'',createdAt:serverTimestamp(),updatedAt:serverTimestamp()};
  try{
    await runTransaction(fs,async tx=>{
      let regRef=null,regSnap=null;
      if(sku && !duplicateByName){ regRef=skuRegistryDocRef(sku); regSnap=await tx.get(regRef); if(regSnap.exists()) throw new Error('SKU ซ้ำ — มีสินค้าอื่นใช้รหัสนี้แล้ว'); }
      if(duplicateByName){
      ...
      tx.set(logDoc,logPayload(duplicateByName?'เพิ่มล็อต':'เพิ่มสินค้า',name,{productId:productDoc.id,eventId,lotNo:initialLot?.lotNo||'',lotId:initialLot?.id||''}));
      tx.set(auditDoc,auditPayload(duplicateByName?'เพิ่มล็อต':'เพิ่มสินค้า',name,{productId:productDoc.id,eventId,logId:logDoc.id,lotNo:initialLot?.lotNo||'',lotId:initialLot?.id||''}));
    });
...
window.saveEditProduct=async(id)=>{
  ...
  const FIELD_LABELS={name:'ชื่อสินค้า',sku:'รหัสสินค้า (SKU)',category:'หมวดหมู่',unit:'หน่วยนับ',min:'จุดเตือนสต๊อกต่ำ',hasExpiry:'มีวันหมดอายุ',expiryDate:'วันหมดอายุ',expiryReminderDays:'แจ้งเตือนล่วงหน้า'};
  const before={name:p.name||'',sku:p.sku||'',category:p.category||'',unit:p.unit||'',min:p.min||0,hasExpiry:!!p.hasExpiry,expiryDate:p.expiryDate||'',expiryReminderDays:p.hasExpiry?getExpiryReminderDays(p):0};
  const after={name,sku,category,unit,min,...expiry};
  const changes=[];
  ...
  try{
    await runTransaction(fs,async tx=>{
      ...
      tx.update(productRef(id),{name,sku,skuKey:newKey,category,unit,min,...expiry,updatedAt:serverTimestamp()});
...
window.stockMove=(id,type)=>{ const p=state.products.find(x=>x.id===id); if(!p) return; if(type==='out'&&!confirmExpiredIssue(p)) return; state.tempMoveImage=null; const locationHtml=type==='in'
...
window.selectScanProduct=(id)=>{
  ...
  const expiryStatus=getExpiryStatus(p);
  $('scanProductSelected').innerHTML=`<div class="scan-selected-product"><span>✅ เลือกแล้ว</span><b>${escapeHtml(p.name)}</b><small>คงเหลือ ${Number(p.stock)||0} ${escapeHtml(p.unit||'')}</small>${expiryStatus.active?`<div class="scan-expiry-warning ${expiryStatus.className}">⏰ ${escapeHtml(expiryStatus.label)} • ${escapeHtml(expiryDateLabel(p))}</div>`:''}</div>`;
...
function applyNewItemDraft(draft){
  ...
  if(p){
    ...
    if(selected){
      const expiryStatus=getExpiryStatus(p);
      selected.innerHTML=`<div class="scan-selected-product"><span>✅ เลือกแล้ว</span><b>${escapeHtml(p.name)}</b><small>คงเหลือ ${Number(p.stock)||0} ${escapeHtml(p.unit||'')}</small>${expiryStatus.active?`<div class="scan-expiry-warning ${expiryStatus.className}">⏰ ${escapeHtml(expiryStatus.label)} • ${escapeHtml(expiryDateLabel(p))}</div>`:''}</div>`;
    }
    renderScanLotSection(p);
...
window.sendApproval=async()=>{
  ...
  const lots=activeProductLots(p); let lotId='',lotNo='',lotExpiryDate='',fefoCorrect=true,fefoOverrideReason='';
  if(type==='out'){
    ...
    lotNo=selectedLot.lotNo; lotExpiryDate=selectedLot.expiryDate||''; fefoCorrect=lotId===lots[0]?.id;
...
function approvalDetailHtml(a, opts={}){
  ...
  return `<div class="card" style="margin:0 0 12px;box-shadow:none;border:1px solid #e5e7eb">
    ...
    ${a.location?`<p class="muted" style="font-size:13px;margin:2px 0 0">📍 ${escapeHtml(a.location)}</p>`:''}${a.lotNo?`<p class="muted" style="font-size:13px">📦 LOT ${escapeHtml(a.lotNo)} • หมดอายุ ${escapeHtml(lotDateLabel(a.lotExpiryDate))}${a.fefoCorrect===false?' • ⚠️ ไม่ใช่ล็อต FEFO':''}</p>`:''}
    ${a.img?`<img class="preview" src="${a.img}">`:''}
    ${expiryLine}
    ${stockLine}
  </div>`;
...
window.approve=async(id)=>{
  ...
  try{
    await runTransaction(fs,async tx=>{
      ...
      const finalFields={action:'อนุมัติ',detail,time:new Date().toLocaleString('th-TH'),updatedAt:serverTimestamp(),location:a.type==='in'?STORE_LOCATION:(a.location||''),reviewerUid,reviewerName,submittedByUid:a.submittedByUid||'',submittedByName:a.submittedByName||'',productId:a.productId,qty:a.qty,unit:a.unit,moveType:a.type,photo:a.img||'',eventId,status:'approved',previousStock:current,newStock,lotId:a.lotId||'',lotNo:a.lotNo||'',lotExpiryDate:a.lotExpiryDate||'',fefoCorrect:a.fefoCorrect!==false,fefoOverrideReason:a.fefoOverrideReason||''};
...
window.saveApproval=async(id)=>{
  ...
  if(type==='out'&&p&&!confirmExpiredIssue(p)) return;
  try{
    await runTransaction(fs,async tx=>{
      const aRef=approvalRef(id),aSnap=await tx.get(aRef);
      if(!aSnap.exists()) throw new Error('รายการนี้ถูกดำเนินการแล้ว');
...
พิมพ์คำว่า "ลบถาวร" เพื่อยืนยัน`);
  ...
  try{
    ...
    toast(imageCleanupFailed?'ลบสินค้าและ SKU แล้ว แต่ลบไฟล์รูปไม่สำเร็จ':'ลบสินค้า SKU และรูปถาวรแล้ว');
    renderTrash();
  }catch(e){
    console.error(e);
...
function renderReport(){
  ...
  const hubReceiveSummary=summarize(periodLogs.filter(l=>l._type==='in'));
  const hubOutSummary=summarize(periodLogs.filter(l=>l._type==='out'));
  ...
  const lowBalanceCount=balanceRows.filter(r=>r.status==='ใกล้หมด'||r.status==='หมด').length;
  const lotExpiryRows=getLotExpiryReportRows();
  const reportHub=`<section class="report-hub-grid">
    <article class="report-hub-card receive"><div class="report-hub-icon">📥</div><div><b>รายงานรับเข้า</b><small>สรุปการรับสินค้าในช่วงวันที่เลือก</small><strong>${hubReceiveSummary.tx} รายการ</strong></div><div class="report-hub-actions"><button class="btn small green" onclick="window.selectReportType('in')">ดูรายงาน</button><button class="btn small light" onclick="window.selectReportType('in');setTimeout(()=>window.exportReportCSV(),80)">Excel</button></div></article>
    <article class="report-hub-card withdraw"><div class="report-hub-icon">📤</div><div><b>รายงานเบิกออก</b><small>สรุปการเบิกสินค้าแยกตามสถานที่</small><strong>${hubOutSummary.tx} รายการ</strong></div><div class="report-hub-actions"><button class="btn small yellow" onclick="window.selectReportType('out')">ดูรายงาน</button><button class="btn small light" onclick="window.selectReportType('out');setTimeout(()=>window.exportReportCSV(),80)">Excel</button></div></article>
    <article class="report-hub-card balance"><div class="report-hub-icon">📦</div><div><b>รายงานสินค้าคงเหลือ</b><small>ยอดคงเหลือปัจจุบัน แยกตามหมวดสินค้า</small><strong>${balanceRows.length} รายการ</strong></div><div class="report-hub-actions"><button class="btn small primary" onclick="window.focusStockBalanceReport()">ดูรายงาน</button><button class="btn small light" onclick="window.exportStockBalanceCSV()">Excel</button></div></article>
    <article class="report-hub-card expiry"><div class="report-hub-icon">⏰</div><div><b>รายงานล็อตที่ต้องระวัง</b><small>ล็อตใกล้หมดอายุหรือหมดอายุแล้ว อยู่ใต้รายงานสต๊อก</small><strong>${lotExpiryRows.length} ล็อต</strong></div><div class="report-hub-actions"><button class="btn small primary" onclick="window.openLotExpiryReport()">ดูรายการ</button><button class="btn small light" onclick="window.goToPage('stock',{filter:'expiry',resetScroll:true})">ไปสต๊อก</button></div></article>
  </section>`;

  const movementSection=['in','out'].includes(state.reportDetailType) ? `<section id="reportMovementSection" class="report-detail-section"><div class="card"><div class="between" style="gap:12px;align-items:flex-start"><div><div class="dashboard-kicker">รายละเอียดรายงาน</div><h2 style="margin:0">${state.reportDetailType==='in'?'รายงานรับเข้า':'รายงานเบิกออก'} • ${escapeHtml(periodLabel)}</h2><p class="muted" style="margin:4px 0 0">แสดงเฉพาะรายงานที่เลือกจากการ์ดด้านบน</p></div><button class="btn small light" onclick="window.clearReportDetail()">ปิดรายละเอียด</button></div><div class="grid" style="margin-top:12px"><div class="stat"><span>${state.reportDetailType==='in'?'รับเข้า':'เบิกออก'}</span><b style="color:${state.reportDetailType==='in'?'#16a34a':'#b45309'}">${state.reportDetailType==='in'?inSummary.tx:outSummary.tx}</b><small>${unitText((state.reportDetailType==='in'?inSummary:outSummary).units)}</small></div></div><div class="row report-export-actions" style="margin-top:12px;gap:8px"><button class="btn small green" onclick="window.exportReportCSV()">📗 Excel/CSV</button><button class="btn small primary" onclick="window.printReportPDF()">📄 PDF / พิมพ์</button></div></div>${state.reportDetailType==='in'?receiveCard:withdrawCards}</section>` : '';

  view.innerHTML=`<div class="report-page"><div class="report-page-head"><div><div class="dashboard-kicker">ศูนย์รายงาน</div><h1>📊 รายงานสต๊อก</h1><p>ดูสรุปรวม ส่งออกเอกสาร และตรวจล็อตที่ต้องระวัง</p></div><button class="btn small light" onclick="window.goToPage('home')">← หน้าแรก</button></div>
...
function renderLotExpiryReportCard(){
  ...
  return `<div id="lotExpiryReportCard" class="card report-export-card" style="align-items:stretch;flex-direction:column;gap:12px">
    ...
    <div class="row report-export-actions"><button class="btn small primary" onclick="window.openLotExpiryReport()">ดูรายการ</button><button class="btn small light" onclick="window.goToPage('stock',{filter:'expiry',resetScroll:true})">ไปสต๊อก</button></div>
  </div>`;
}
...
window.selectReportType=(filter='all')=>{
  ...
  state.reportDetailType=['in','out'].includes(safeFilter)?safeFilter:'';
  saveUiState();
  renderReport();
  setTimeout(()=>document.getElementById('reportMovementSection')?.scrollIntoView({behavior:'smooth',block:'start'}),60);
};
...
window.openLotExpiryReport=()=>{
  const rows=getLotExpiryReportRows();
  const body=rows.map(({product,lot,displayNo,meta})=>`<button class="lot-expiry-report-row lot-${meta.key}" onclick="hideModal();window.viewProductLots('${product.id}')">
    <div><b>${escapeHtml(displayNo)}</b><small>${escapeHtml(product.name||'-')} • คงเหลือ ${Number(lot.qty)||0} ${escapeHtml(product.unit||'')}</small></div>
    <div><strong>${escapeHtml(lotDateLabel(lot.expiryDate))}</strong><span>${escapeHtml(meta.label)}</span></div>
  </button>`).join('') || `<div class="dashboard-empty ok">✅ ยังไม่มีล็อตที่ต้องระวัง</div>`;
  openModal('รายงานล็อตใกล้หมดอายุ',`<div class="lot-expiry-report"><p class="muted">แสดงล็อตที่หมดอายุแล้ว / หมดอายุวันนี้ / เหลือไม่เกิน 30 วัน</p>${body}<button class="btn light full" style="margin-top:12px" onclick="hideModal();window.goToPage('stock',{filter:'expiry',resetScroll:true})">เปิดหน้าสต๊อกใกล้หมดอายุ</button></div>`);
};
...
function renderHistory(){
  ...
  const renderLog=(l)=>{
    ...
    const photoThumb=l.photo ? `<img class="history-entry-thumb" src="${escapeHtml(l.photo)}" alt="รูปหลักฐาน ${escapeHtml(l.detail||'รายการ')}" loading="lazy">` : '';
    return `<article class="history-entry history-entry-clickable" role="button" tabindex="0" onclick="window.viewHistoryEntry('${escapeHtml(l.id||'')}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();window.viewHistoryEntry('${escapeHtml(l.id||'')}')}">
      <div class="history-entry-content">
        ...
        <div class="history-entry-open-hint">แตะเพื่อดูรายละเอียด${l.photo?'และรูปภาพ':''} ›</div>
      </div>
      ${photoThumb}
    </article>`;
  };
...
window.viewHistoryEntry=function(logId){
  ...
  const photo=l.photo?`<section class="history-detail-section"><h3>รูปหลักฐาน</h3><button class="history-detail-photo-button" type="button" onclick="window.openHistoryPhoto('${escapeHtml(l.id||'')}')"><img class="history-detail-photo" src="${escapeHtml(l.photo)}" alt="รูปหลักฐาน ${escapeHtml(productName)}"><span>แตะรูปเพื่อขยายเต็มหน้าจอ</span></button></section>`:`<section class="history-detail-section"><h3>รูปหลักฐาน</h3><div class="history-no-photo">📷 ไม่มีรูปภาพแนบในรายการนี้</div></section>`;
  openModal(`รายละเอียดรายการ`, `<div class="history-detail-modal">
    ...
    <section class="history-detail-section"><h3>ข้อมูลรายการ</h3><div class="history-detail-grid">
      <div><span>${l.moveType==='in'?'ผู้รับสินค้า':l.moveType==='out'?'ผู้เบิกสินค้า':'ผู้ดำเนินการ'}</span><b>${escapeHtml(requester)}</b></div>
      <div><span>สถานที่</span><b>${escapeHtml(l.location||'-')}</b></div>
...
window.openHistoryPhoto=function(logId){
  const l=state.logs.find(item=>String(item.id||'')===String(logId||''));
  if(!l?.photo) return toast('ไม่พบรูปภาพ');
  openModal('รูปหลักฐาน',`<div class="history-photo-viewer"><img src="${escapeHtml(l.photo)}" alt="รูปหลักฐาน"><button class="btn light full" onclick="hideModal()">ปิดรูปภาพ</button></div>`);
};
...
function roleLabel(value='staff'){
  return ({admin:'ผู้ดูแลระบบ',manager:'Manager',supervisor:'Supervisor',captain:'Captain',staff:'Staff'}[value]||value||'Staff');
}
  ...
  view.innerHTML = `<div class="manual-page visual-manual v34202">
    <section class="manual-visual-hero v34202">
      <div class="manual-visual-copy">
        <span class="manual-kicker">คู่มือย่อในระบบ + ลิงก์คู่มือเต็มหน้า</span>
        <h1>CHEE CHAN STOCK</h1>
        <p>หน้านี้เป็นคู่มือย่อสำหรับเปิดดูเร็ว ส่วนคู่มือเต็มหน้าเป็นฉบับละเอียดที่สุด พร้อมภาพตัวอย่าง ขั้นตอนทำตามจริง และปุ่มพิมพ์เป็น PDF</p>
        <div class="manual-hero-actions">
          ...
          <button class="btn" onclick="window.open('manual.html','_blank')">🌐 เปิดคู่มือเต็มหน้าแบบละเอียด</button>
        </div>
      </div>
    ...
    <section class="manual-role-grid visual-roles">
      <article class="role-admin"><div class="manual-role-icon">👑</div><h3>ผู้ดูแลระบบ</h3><p>ดูแลหลังบ้าน จัดการระบบ ผู้ใช้ สิทธิ์ สำรองข้อมูล และแก้ปัญหาทางเทคนิค</p></article>
      <article class="role-lead"><div class="manual-role-icon">🧑‍💼</div><h3>Manager</h3><p>ดูแลภาพรวม อนุมัติ ตรวจสอบรายงาน และจัดการสิทธิ์พนักงาน</p></article>
      <article class="role-lead"><div class="manual-role-icon">👥</div><h3>Supervisor / Captain</h3><span class="role-equal-badge">สิทธิ์เท่ากัน</span><p>รับเข้า เบิกออก อนุมัติ แก้ไขล็อต ดูประวัติ รายงาน และจัดการสิทธิ์พนักงาน</p></article>
      ...
      <article class="role-admin manual-employee-note"><div class="manual-role-icon">📝</div><h3>การเพิ่มพนักงานใหม่</h3><p>หากต้องการเพิ่มพนักงานเข้าสู่ระบบ กรุณาแจ้งผู้ดูแลระบบ (Admin) เพื่อเพิ่มชื่อและกำหนดสิทธิ์การใช้งาน</p></article>
    </section>
    ...
    <section class="manual-process-board">
      ...
      <div class="manual-process-grid">
        ...
        ${manualProcessStep('3','📤','เบิกออก','เลือกสินค้า จำนวน สถานที่ และแนบหลักฐาน')}
    ...
    <section class="manual-section-list visual-section-list" id="manualSections">
      ${manualDetail('1. ระบบนี้สร้างมาเพื่ออะไร','overview purpose ประโยชน์ จัดการสินค้า','🎯',`
        ...
        <div class="manual-deep-grid">
          ...
          ${manualDeepCard('📤','เบิกออกตามสถานที่','บันทึกว่าสินค้าออกไปใช้ที่ไหน ใครเบิก จำนวนเท่าไหร่ และเลือกล็อตที่ถูกต้อง',manualScreenIssue())}
      ...
      ${manualDetail('2. สิทธิ์ผู้ใช้งาน','role admin manager captain staff สิทธิ์','👥',`
        <div class="manual-role-task-grid">
          <div><b>ผู้ดูแลระบบ</b><ul><li>ดูแลระบบหลังบ้าน</li><li>จัดการผู้ใช้และสิทธิ์หลัก</li><li>เพิ่มพนักงานใหม่</li><li>สำรองข้อมูลและแก้ปัญหาทางเทคนิค</li></ul></div>
          <div><b>Manager</b><ul><li>รับเข้า / เบิกออก</li><li>อนุมัติรายการ</li><li>ดูประวัติและรายงาน</li><li>จัดการสิทธิ์พนักงาน</li></ul></div>
          <div><b>Supervisor / Captain <span class="role-badge">สิทธิ์เท่ากัน</span></b><ul><li>รับเข้า / เบิกออก</li><li>อนุมัติรายการ</li><li>แก้ไขล็อตและวันหมดอายุ</li><li>จัดการสิทธิ์พนักงาน</li></ul></div>
        ...
        <p class="manual-callout-warn">การเพิ่มพนักงานใหม่เป็นสิทธิ์เฉพาะผู้ดูแลระบบ หากต้องการเพิ่มพนักงาน กรุณาแจ้ง Admin เพื่อเพิ่มชื่อและกำหนดสิทธิ์การใช้งาน</p>` )}

      ${manualDetail('3. วิธีเข้าสู่ระบบ','login password chartered เข้าสู่ระบบ','🔐',`
        <div class="manual-deep-grid">
          ${manualDeepCard('1','เปิดเว็บไซต์','เข้าเว็บไซต์ CHEE CHAN STOCK จากมือถือ แท็บเล็ต หรือคอมพิวเตอร์',manualScreenLogin('open'))}
        ...
        <ol class="manual-step-list"><li>เปิดเว็บไซต์ CHEE CHAN STOCK</li><li>กรอก Username เช่น jettaphonj</li><li>กรอกรหัสผ่าน</li><li>กดเข้าสู่ระบบ</li><li>ถ้าระบบให้เปลี่ยนรหัสผ่าน ให้ตั้งรหัสใหม่ก่อนใช้งาน</li></ol>
        <p class="manual-callout-warn">ถ้าเข้าระบบไม่ได้ ให้ตรวจตัวสะกด Username/Password ก่อน แล้วติดต่อ Admin เพื่อตรวจสิทธิ์</p>`)}
      ...
      ${manualDetail('6. การเพิ่มสินค้าใหม่','add product เพิ่มสินค้าใหม่','➕',`
        ...
        <ol class="manual-step-list"><li>กดรายการใหม่</li><li>เลือกเพิ่มสินค้า</li><li>กรอกชื่อสินค้า</li><li>เลือกหมวดหมู่และหน่วยนับ</li><li>เพิ่มรูปสินค้า</li><li>กดบันทึก</li></ol>
      ...
      ${manualDetail('8. การเบิกออกสินค้า','issue withdraw เบิกออก FEFO เหตุผล','📤',`
        ...
        <ol class="manual-step-list"><li>กดรายการใหม่</li><li>เลือก เบิกออก</li><li>เลือกสินค้า</li><li>เลือกล็อตที่ต้องการเบิก</li><li>ระบุจำนวนและสถานที่ใช้งาน</li><li>แนบรูปหลักฐานถ้ามี</li><li>ตรวจสอบแล้วกดบันทึก</li></ol>
      ...
      ${manualDetail('11. ประวัติรายการ','history ประวัติ รูปหลักฐาน','📋',`
        <div class="manual-deep-grid">
          ...
          ${manualDeepCard('รายละเอียดพร้อมรูป','กดการ์ดเพื่อดูภาพใหญ่','ดูรูปหลักฐาน ผู้รับเข้า/ผู้เบิก สถานที่ ผู้อนุมัติ ล็อต วันเวลา และหมายเหตุ',manualScreenHistoryDetail())}
        </div>
      ...
      ${manualDetail('13. ปัญหาที่พบบ่อย','troubleshooting ปัญหา ไม่มีสิทธิ์ QR cache rules','❓',`
        <div class="manual-faq-grid v34202">
          <div><b>สแกน QR แล้วไม่เจอสินค้า</b><span>สินค้าอาจถูกลบแล้ว หรือ QR เป็นใบเก่า ให้สร้าง QR ใหม่จากหน้าสินค้า</span></div>
          <div><b>หน้าเว็บไม่อัปเดต</b><span>รีเฟรชหน้า ล้างแคช Safari/Chrome หรือเปิดเว็บใหม่อีกครั้ง</span></div>
          <div><b>ไม่มีสิทธิ์เข้าถึงข้อมูล</b><span>บัญชีอาจยังไม่ได้รับสิทธิ์ ให้ติดต่อ Admin หรือผู้ดูแลระบบ</span></div>
    ...
    <section class="manual-summary">
      <h2>สรุปสั้น ๆ</h2>
      <p><b>CHEE CHAN STOCK</b> ช่วยให้สต๊อกเป็นระบบ รู้ยอดคงเหลือ ตรวจล็อตและวันหมดอายุได้ ลดความผิดพลาดจากการจดมือ และตรวจสอบย้อนหลังพร้อมหลักฐานได้ชัดเจนขึ้น</p>
    </section>
  </div>`;
}
...
function manualScreenIssue(){return manualScreen('เบิกออกสินค้า',`<div class="manual-demo-row"><em>🍚</em><div><b>ข้าวผัด</b><span>คงเหลือ 90 จาน</span></div></div><div class="manual-demo-lot"><b>⭐ ข้าวผัด-001</b><span>ควรเบิกก่อน • เหลือ 90 จาน</span></div><div class="manual-demo-input">สถานที่: TheView</div><button class="manual-demo-button">บันทึกเบิกออก</button>`)}
...
window.manageMembers=async()=>{ if(!requireAdmin()) return;
  ...
  const rows=state.members.map(m=>`<div class="card member-row-card" style="box-shadow:none;border:1px solid var(--line);margin:8px 0">
    ...
    ${m.uid===state.user?.uid?'':`<button class="btn red full" onclick="window.deleteMemberProfile('${m.uid}')">🗑️ ลบสมาชิกนี้ออกจากระบบ</button>`}
  </div>`).join('');
  openModal('จัดการสมาชิกและตำแหน่ง',`<div class="member-admin-tools">
    ...
    <p class="note">พนักงานใหม่จะใช้รหัสผ่านเริ่มต้น <b>chartered</b> และระบบจะบังคับเปลี่ยนรหัสผ่านเมื่อเข้าใช้งานครั้งแรก</p><p class="note">ปุ่มลบจะลบโปรไฟล์สมาชิกออกจากระบบ ทำให้เข้าใช้งานไม่ได้ ส่วนบัญชี Firebase Authentication หากต้องการลบถาวรให้ลบผ่าน Firebase Console</p>
  </div>${rows||'<p class="muted">ยังไม่มีสมาชิก</p>'}`);
};

function memberRoleOptions(selected='staff'){
  return ['staff','captain','supervisor','manager'].map(r=>`<option value="${r}" ${selected===r?'selected':''}>${escapeHtml(roleLabel(r))}</option>`).join('');
}
...
async function createMemberViaClient(payload){
  ...
  const role=['staff','captain','supervisor','manager'].includes(payload.role) ? payload.role : 'staff';
  ...
  for(let i=0;i<30;i++){
    ...
    try{
      ...
      await addLog('เพิ่มพนักงานใหม่',`${displayName} • ${username} • ${roleLabel(role)}`);
      await signOut(memberCreateAuth).catch(()=>{});
      return {uid:createdUser.uid,displayName,username,role,initialPassword:DEFAULT_PASSWORD};
    }catch(e){
      lastError=e;
...
function memberCreateResultMarkup(results=[]){
  return `<div class="member-create-results">${results.map(r=>`<div class="card" style="box-shadow:none;border:1px solid var(--line);margin:8px 0">
    ...
    ${r.ok?`<div>Username: <b>${escapeHtml(r.username||'-')}</b></div><div>รหัสเริ่มต้น: <b>${escapeHtml(r.password||DEFAULT_PASSWORD)}</b></div><div class="muted">ตำแหน่ง: ${escapeHtml(roleLabel(r.role||'staff'))}</div>`:`<div class="muted">${escapeHtml(r.error||'สร้างไม่สำเร็จ')}</div>`}
  </div>`).join('')}</div>`;
}
...
window.createMemberNow=async()=>{ if(!requireAdmin()) return;
  ...
  try{
    ...
    const result={ok:true,displayName:data.displayName||payload.displayName,username:data.username,password:data.initialPassword||DEFAULT_PASSWORD,role:data.role||payload.role};
...
window.deleteMemberProfile=async(uid)=>{ if(!requireAdmin()) return;
  ...
  const confirmText=`ต้องการลบสมาชิก “${label}” ออกจากระบบใช่หรือไม่?

ผลลัพธ์:
- สมาชิกจะหายจากรายชื่อในระบบ
- สมาชิกจะเข้าใช้งานระบบไม่ได้
- ประวัติรายการเก่าจะยังเก็บไว้เพื่อการตรวจสอบย้อนหลัง
- หากต้องการลบบัญชี Firebase Authentication ถาวร ให้ลบผ่าน Firebase Console`;
  if(!confirm(confirmText)) return;
  try{
    await addLog('ลบสมาชิกออกจากระบบ',`${label} • @${member.username||'-'} • ${roleLabel(member.role||'staff')}`);
    await deleteDoc(memberRef(uid));
    toast('ลบสมาชิกออกจากระบบแล้ว');
...
function openModal(t,b){
  ...
  if(isQrModal){ modal.scrollTop=0; }
...
function hideModal(){
  ...
  $('modal').classList.remove('qr-modal');
...
function ensureNetworkStatusIndicator(){
  ...
  // Append directly to hero so absolute positioning always uses the card itself.
  hero.appendChild(el);
  return el;
}
