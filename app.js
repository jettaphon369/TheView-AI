...
window.exportBackup=()=>{
  ...
  const data={version:'30.3-visible-tools-fix',workspace:'main',products:clean(state.products),approvals:clean(state.approvals),logs:clean(state.logs),auditLogs:clean(state.auditLogs),exportedAt:new Date().toISOString(),exportedBy:state.profile?.displayName||state.profile?.username||''};
