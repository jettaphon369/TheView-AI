...
function unbindRealtime(){
  ...
  state.products=[]; state.approvals=[]; state.logs=[]; state.auditLogs=[]; logsCursor=null; logsHasMore=true; logsLoadingMore=false;
}
...
async function getReportSourceLogs(){
  // Export ต้องไม่พึ่งเฉพาะข้อมูล realtime ที่จำกัดไว้ จึงดึงข้อมูลจาก Firestore ตอนสั่ง Export เท่านั้น
  const snap=await getDocs(query(userPath('logs'),orderBy('createdAt','desc')));
...
window.printReportPDF=async()=>{
  ...
  let rows=[];
  try{ rows=buildCurrentReportRows(await getReportSourceLogs()); }catch(e){ console.error(e); w.close(); alert('โหลดข้อมูลรายงานไม่สำเร็จ'); return; }
  if(!rows.length){ alert('ไม่มีข้อมูลในช่วงเวลาที่เลือก'); return; }
  const period=state.reportMode==='month'
    ? `เดือน ${state.reportMonth}`
    : state.reportMode==='range'
