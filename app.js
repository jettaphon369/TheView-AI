...
function ensureAppShellStyles(){
  ...
  style.textContent=`
    ...
    .bottom-nav button span{font-size:25px!important}
...
function setupAppShell(){
  ...
  if(nav){
    nav.innerHTML=`
      <button data-page="home" class="active"><span>🏠</span>หน้าแรก</button>
      <button data-page="stock"><span>📦</span>สต๊อก</button>
      <button data-page="scan" class="nav-create"><span>＋</span>รายการใหม่</button>
      <button data-page="history"><span>📋</span>ประวัติ</button>
      <button data-page="profile"><span>👤</span>โปรไฟล์</button>
    `;
  }
...
window.exportBackup=()=>{
  ...
  const data={version:'28.5',workspace:'main',products:clean(state.products),approvals:clean(state.approvals),logs:clean(state.logs),exportedAt:new Date().toISOString(),exportedBy:state.profile?.displayName||state.profile?.username||''};
