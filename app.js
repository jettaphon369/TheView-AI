...
// ---------- v32.7: กู้ตำแหน่งเลื่อนแบบรอข้อมูล Realtime โหลดครบ ----------
...
window.exportBackup=()=>{
  ...
  const data={version:'32.7',workspace:'main',products:clean(state.products),approvals:clean(state.approvals),logs:clean(state.logs),auditLogs:clean(state.auditLogs),exportedAt:new Date().toISOString(),exportedBy:state.profile?.displayName||state.profile?.username||''};
