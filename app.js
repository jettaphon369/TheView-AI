...
const state = { user:null, profile:null, members:[], page:'home', products:[], approvals:[], logs:[], selectedImage:null, imageMode:null, viewProductId:null, tempMoveImage:null, tempProductImage:null, stockFilter:'all', reportMode:'day', reportFilter:'all', reportDate:'', reportMonth:'' };
const view = $('view');
...
function renderReport(){
  ...
  let periodLogs, periodLabel;
  if(state.reportMode==='month'){
    const [y,m] = state.reportMonth.split('-').map(Number);
    periodLogs = movementLogs.filter(l=> l._d.getFullYear()===y && (l._d.getMonth()+1)===m);
    periodLabel = `เดือน ${pad2(m)}/${y}`;
  } else {
    const [y,m,d] = state.reportDate.split('-').map(Number);
    periodLogs = movementLogs.filter(l=> l._d.getFullYear()===y && (l._d.getMonth()+1)===m && l._d.getDate()===d);
  ...
  const receiveCard = receiveLogs.length ? `<div class="card" style="border:1px solid #bbf7d0">
    <div class="between"><div><div class="muted">รับเข้าสินค้า</div><h2 style="margin:2px 0;color:#15803d">📥 ${STORE_LOCATION}</h2></div><span class="pill ok">${receiveSummary.tx} รายการ</span></div>
  ...
  const withdrawCards=locNames.map(loc=>{
    ...
    return `<div class="card" style="border:1px solid #fde68a"><div class="between"><div><div class="muted">เบิกออกไปยัง</div><h2 style="margin:2px 0;color:#b45309">📍 ${escapeHtml(loc)}</h2></div><span class="pill warn">${sm.tx} รายการ</span></div><div style="margin-top:8px">${productRows(sm.products)}</div><div class="between" style="margin-top:10px;padding-top:8px;border-top:2px solid #fde68a"><span class="muted">รวมเบิกออก</span><b>${unitText(sm.units)}</b></div></div>`;
  ...
  <div class="card"><div class="row" style="gap:8px;margin-bottom:10px"><button class="btn small ${state.reportMode==='day'?'primary':'light'}" onclick="window.setReportMode('day')">รายวัน</button><button class="btn small ${state.reportMode==='month'?'primary':'light'}" onclick="window.setReportMode('month')">รายเดือน</button></div>${controls}<div class="row" style="gap:8px;margin-top:12px;flex-wrap:wrap"><button class="btn small ${state.reportFilter==='all'?'primary':'light'}" onclick="window.setReportFilter('all')">ทั้งหมด</button><button class="btn small ${state.reportFilter==='in'?'green':'light'}" onclick="window.setReportFilter('in')">รับเข้า</button><button class="btn small ${state.reportFilter==='out'?'yellow':'light'}" onclick="window.setReportFilter('out')">เบิกออก</button></div></div>
  <div class="card"><h2 style="margin:0">สรุป ${escapeHtml(periodLabel)}</h2><div class="grid" style="margin-top:12px"><div class="stat"><span>ทั้งหมด</span><b>${allSummary.tx}</b><small>${unitText(allSummary.units)}</small></div><div class="stat"><span>รับเข้า</span><b style="color:#16a34a">${inSummary.tx}</b><small>${unitText(inSummary.units)}</small></div><div class="stat"><span>เบิกออก</span><b style="color:#b45309">${outSummary.tx}</b><small>${unitText(outSummary.units)}</small></div></div></div>
...
window.exportBackup=()=>{
  ...
  const data={version:'25.4',workspace:'main',products:clean(state.products),approvals:clean(state.approvals),logs:clean(state.logs),exportedAt:new Date().toISOString(),exportedBy:state.profile?.displayName||state.profile?.username||''};
