...
import { getAuth, onAuthStateChanged, signInWithEmailAndPassword, signOut, setPersistence, browserLocalPersistence, updatePassword } from 'https://www.gstatic.com/firebasejs/10.12.5/firebase-auth.js';
...
onAuthStateChanged(auth, async user=>{
  ...
  view.innerHTML = '<div class="card"><h2>กำลังเตรียมระบบ…</h2><p class="muted">กำลังตรวจสอบบัญชีและโหลดข้อมูลส่วนกลาง</p></div>';
  try{
    ...
    // แสดงหน้าแรกทันที ไม่ปล่อยให้หน้าขาว แม้การสร้างข้อมูลตัวอย่างจะล้มเหลว
    render();
    bindRealtime();

    try{ await seedIfEmpty(); }
    catch(seedError){
      console.warn('ข้ามการสร้างข้อมูลเริ่มต้น:', seedError);
      if(seedError?.code==='permission-denied') toast('เข้าสู่ระบบแล้ว แต่ยังไม่มีสิทธิ์สร้างข้อมูลตัวอย่าง');
    }

    if(state.profile.mustChangePassword) setTimeout(()=>window.openFirstPasswordChange(),300);
  }catch(error){
