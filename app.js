...
const completeFirstPasswordChange = httpsCallable(functions,'completeFirstPasswordChange');
...
const BUILD_VERSION='34.19.7-QR-DELETE-REVOKE-FIX';
...
function canManageLots(){ return isAdmin() || isManagerRole() || isCaptain() || hasPermission('canManageLots'); }
...
function syncDesktopWorkspaceV34163(){
  const pageLabel={home:'ภาพรวมคลังสินค้า',stock:'คลังสินค้า',scan:'สร้างรายการใหม่',approval:canApprove()?'รายการอนุมัติ':'รายการรอตรวจ',report:'รายงานสต๊อก',history:'ประวัติการใช้งาน',profile:'บัญชีผู้ใช้',trash:'ถังขยะ',productDetail:'รายละเอียดสินค้า'}[state.page]||'ระบบคลังสินค้า';
...
function productQrPayload(product={}){
  ...
  url.searchParams.set('v','34.19.7');
  return url.toString();
}
...
window.saveNewPassword=async(first)=>{
  ...
  try{
    if(first){
      // v34.16.2 SECURITY:
      // Cloud Function เป็นผู้เปลี่ยนรหัสใน Firebase Auth และปลด mustChangePassword
      // Client ไม่มีสิทธิ์แก้ field ประตูรหัสผ่านอีกต่อไป
      await completeFirstPasswordChange({newPassword:a});

      // Admin SDK เปลี่ยนรหัสจากฝั่ง Server ซึ่งอาจยกเลิก refresh token เดิม
      // ลงชื่อเข้าใช้อีกครั้งด้วยรหัสใหม่เพื่อออก session ที่สมบูรณ์
      await signInWithEmailAndPassword(auth,user.email,a);
      ...
      : code==='auth/weak-password' || code==='functions/invalid-argument'
        ...
        : code==='auth/requires-recent-login'
          ...
          : code==='functions/not-found'
            ? 'ยังไม่ได้ Deploy Cloud Function สำหรับปลดประตูรหัสผ่าน กรุณา Deploy functions ก่อนใช้งานบัญชีใหม่'
            : code==='functions/unauthenticated'
              ? 'เซสชันหมดอายุ กรุณาออกจากระบบแล้วเข้าสู่ระบบใหม่'
              : code==='functions/failed-precondition'
                ? (e?.message||'บัญชีนี้ไม่อยู่ในสถานะเปลี่ยนรหัสผ่านครั้งแรก')
                : code==='functions/unavailable'
                  ? 'เชื่อมต่อระบบเปลี่ยนรหัสผ่านไม่ได้ กรุณาตรวจสอบอินเทอร์เน็ตแล้วลองใหม่'
