...
function renderHome(){
  ...
  const approvalCard=canApprove()?`
    <div class="home-priority-card ${pending?'has-pending':'clear'}" onclick="window.goToPage('approval')" role="button" tabindex="0">
      ...
      <div class="home-priority-copy">
        ...
        <b>${pending}</b>
        <small>${pending?'มีรายการที่ต้องตรวจสอบและอนุมัติ':'ไม่มีรายการรออนุมัติ'}</small>
      </div>
  ...
  view.innerHTML=`<h1>หน้าแรก</h1>
    ...
    <div class="card home-system-card">
      <h2>สถานะระบบ</h2>
      ${canApprove()?`<button class="home-system-row ${pending?'warn':'ok'}" onclick="window.goToPage('approval')">
        <span>${pending?'🔔':'✅'} รายการรออนุมัติ</span><b>${pending} รายการ</b>
      </button>`:''}
...
function renderApproval(){
  ...
  view.innerHTML=`<h1>รายการรออนุมัติ</h1>
...
window.exportBackup=()=>{
  ...
  const data={version:'27.8',workspace:'main',products:clean(state.products),approvals:clean(state.approvals),logs:clean(state.logs),exportedAt:new Date().toISOString(),exportedBy:state.profile?.displayName||state.profile?.username||''};
